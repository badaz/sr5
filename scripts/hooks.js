...
export const registerHooks = function () {
  Hooks.once("init", async function () {
    ...
    SR5_SystemHelpers.srLogPublic(
      `Remember: Never, ever, cut a deal with a dragon!`
    );
    ...
    switch (uitheme) {
      case "SR6": {
        root.setAttribute("class", "theme-sr6");
        break;
      }
      default: {
        root.setAttribute("class", "theme-sr5");
        CONFIG.TinyMCE.content_css = "systems/sr5/css/styles.css";
      }
    }

    CONFIG.TinyMCE.content_css = CONFIG.TinyMCE.content_css.concat(
      "systems/sr5/css/styles.css"
    );
  ...
  Hooks.once("ready", function () {
    ...
    const needsMigration =
      !currentVersion ||
      isNewerVersion(NEEDS_MIGRATION_VERSION, currentVersion); //isNewerVersion(v0, v1)
  ...
  Hooks.on(
    ...
    SR5CombatTracker.addCombatTrackerContextOptions
  );
  Hooks.on("renderCombatTracker", (app, html, data) =>
    SR5CombatTracker.renderCombatTracker(app, html, data)
  );
  ...
  Hooks.on("renderFolderConfig", (dialog, html) => {
    ...
    if (
      ...
      !html.find(`input[type=text]`)[0].value
    ) {
      html.find(`input[type=text]`)[0].value =
        html.find(`input[type=text]`)[0].placeholder;
  ...
  Hooks.on("preCreateToken", (tokenDocument) => {
    if (tokenDocument.data.img.includes("systems/sr5/img/actors/"))
  ...
  Hooks.on("createCombatant", (combatant) => {
    ...
    combatant.update({
      ...
      "flags.sr5.currentInitRating":
        combatant.actor.data.data.initiatives[key].value,
      "flags.sr5.currentInitDice":
        combatant.actor.data.data.initiatives[key].dice.value,
    });
  ...
  Hooks.on("updateActor", async (document, data, options, userId) => {
    ...
    let astralVisionEffect = document.effects.find(
      (e) => e.data.origin === "handleVisionAstral"
    );
    if (document.data.data.visions?.astral.isActive) {
      ...
      if (astralVisionEffect)
        await document.deleteEmbeddedDocuments("ActiveEffect", [
          astralVisionEffect.id,
        ]);
    }
  ...
  Hooks.on("deleteActiveEffect", (effect) => {
    if (effect.data.flags.core?.statusId === "astralInit")
