...
export class SR5Item extends Item {
  static async create(data, options) {
    if (!data.img) {
      data.img = `systems/sr5/assets/img/items/${data.type}.svg`;
    }
  ...
  prepareData() {
    ...
    switch (itemData.type) {
      case "itemWeapon":
        ...
        if (typeof data.accessory === "object")
          data.accessory = Object.values(data.accessory);
        if (data.damageElement === "toxin")
          SR5_UtilityItem._handleWeaponToxin(data);
        if (data.ammunition.value > data.ammunition.max)
          data.ammunition.value = data.ammunition.max;
        ...
        data.conditionMonitors.matrix.value =
          Math.ceil(data.deviceRating / 2) + 8;
      ...
      case "itemSin":
        if (typeof data.license === "object")
          data.license = Object.values(data.license);
      ...
      case "itemSprite":
        data.conditionMonitors.matrix.value =
          Math.ceil(data.itemRating / 2) + 8;
        SR5_EntityHelpers.GenerateMonitorBoxes(data, "matrix");
        break;
      case "itemVehicle":
        if (typeof data.mount === "object")
          data.mount = Object.values(data.mount);
        SR5_UtilityItem._handleVehicle(data);
        SR5_UtilityItem._handleItemPrice(data);
        SR5_UtilityItem._handleItemAvailability(data);
        data.conditionMonitors.matrix.value =
          Math.ceil(data.attributes.pilot / 2) + 8;
        if (data.type === "drone")
          data.conditionMonitors.condition.base = Math.ceil(
            data.attributes.body / 2 + 6
          );
        else
          data.conditionMonitors.condition.base = Math.ceil(
            data.attributes.body / 2 + 12
          );
      ...
      case "itemDrug":
        ...
        for (let key of Object.keys(SR5.propagationVectors)) {
          if (data.vector[key]) {
            data.vector.value.push(
              game.i18n.localize(SR5.propagationVectors[key])
            );
          }
        }
      default:
    ...
    switch (this.data.type) {
      case "itemAugmentation":
        ...
        if (data.marks.length) {
          for (let m of data.marks) {
            tags.push(
              game.i18n.localize("SR5.Mark") +
                game.i18n.localize(`SR5.Colons`) +
                ` ${m.ownerName} [${m.value}]`
            );
          }
        }
        if (data.isSlavedToPan) {
          ...
          tags.push(
            game.i18n.localize("SR5.DeviceSlavedToPan") + ` (${panMaster.name})`
          );
        }
        break;
      case "itemWeapon":
        if (data.category === "rangedWeapon") {
          tags.push(
            ...
            game.i18n.localize(`SR5.WeaponModesShort`) +
              game.i18n.localize(`SR5.Colons`) +
              ` ${data.firingMode.value}`,
            game.i18n.localize(`SR5.RecoilCompensationShort`) +
              game.i18n.localize(`SR5.Colons`) +
              ` ${data.recoilCompensation.value}`,
            game.i18n.localize(`SR5.WeaponRange`) +
              game.i18n.localize(`SR5.Colons`) +
              ` ${data.range.short.value}/${data.range.medium.value}/${data.range.long.value}/${data.range.extreme.value}` +
              game.i18n.localize(`SR5.MeterUnit`),
            game.i18n.localize(`SR5.Ammunition`) +
              game.i18n.localize(`SR5.Colons`) +
              ` ` +
              game.i18n.localize(lists.allAmmunitionTypes[data.ammunition.type])
          );
          if (data.accessory) {
            for (let a of data.accessory) {
              accessories.push(`${a.name}: ${a.data?.gameEffect}`);
              tags.push([
                game.i18n.localize(lists.weaponAccessories[a.name]),
                a.gameEffects,
              ]);
            }
          }
          ...
          tags.push(
            ...
            game.i18n.localize(`SR5.WeaponReach`) +
              game.i18n.localize(`SR5.Colons`) +
              ` ${data.reach.value}`
          );
      ...
      case "itemSpell":
        ...
        switch (data.category) {
          case "combat":
            tags.push(
              game.i18n.localize(lists.spellCombatTypes[data.subCategory])
            );
            break;
          case "detection":
            tags.push(
          ...
          case "manipulation":
            tags.push(
              game.i18n.localize(lists.spellManipulationTypes[data.subCategory])
            );
        ...
        if (this.data.type === "itemSpell")
          tags.push(
            game.i18n.localize(`SR5.SpellDrain`) +
              game.i18n.localize(`SR5.Colons`) +
              ` ${data.drainModifier}`
          );
        break;
  ...
  reloadAmmo() {
    ...
    if (this.actor.isToken)
      ownerActor = SR5_EntityHelpers.getRealActorFromID(this.actor.token.id);
    ...
    let itemAmmo = ownerActor.items.find(
      (item) =>
        item.type === "itemAmmunition" &&
        item.data.data.type === data.data.ammunition.type &&
        item.data.data.class === data.data.type
    );
    if (itemAmmo) {
        ...
        Dialog.confirm({
          ...
          content:
            ...
            game.i18n.format("SR5.DIALOG_WarningNoMoreAmmo", {
              actor: ownerActor.name,
              ammoType: game.i18n.localize(
                lists.allAmmunitionTypes[data.data.ammunition.type]
              ),
              weaponType: game.i18n.localize(
                lists.rangedWeaponTypes[data.data.type]
              ),
              itemName: data.name,
            }) +
            "</p>",
