...
export class SR5Actor extends Actor {
  ...
  static async create(data, options) {
    if (!data.img) {
      data.img = `systems/sr5/assets/img/actors/${data.type}.svg`;
    }
            ...
            buttons: {
              ok: {
                ...
                callback: async (dialog) => {
                  ...
                  baseItems = await SR5_CompendiumUtility.getBaseItems(
                    data.type,
                    spiritType,
                    spiritForce
                  );
  ...
  async _preCreate(createData, options, user) {
    ...
    if (this.type === "actorSpirit" && this.data.data.creatorId !== "")
      actorLink = true;
    if (this.type === "actorDrone" && this.data.data.creatorId !== "")
      actorLink = true;
    if (this.type === "actorSprite" && this.data.data.creatorId !== "")
      actorLink = true;
    ...
    switch (this.type) {
      ...
      case "actorAgent":
        ...
        let effect =
          SR5_CharacterUtility.generateInitiativeEffect("matrixInit");
      ...
      default:
        SR5_SystemHelpers.srLog(
          1,
          `Unknown '${this.type}' type in 'base _preCreate()'`
        );
    }
  }
  ...
  prepareBaseData() {
    ...
    switch (actor.type) {
      ...
      default:
        SR5_SystemHelpers.srLog(
          1,
          `Unknown '${actor.type}' actor type in prepareBaseData()`
        );
    }
    ...
    switch (actor.type) {
      case "actorDrone":
        if (actor.flags.sr5?.vehicleControler)
          SR5_CharacterUtility.applyAutosoftEffect(actor);
      ...
      default:
        SR5_SystemHelpers.srLog(
          1,
          `Unknown '${actor.type}' actor type in prepareDerivedData()`
        );
    }
  }
  ...
  prepareEmbeddedDocuments() {
    ...
    for (let i of actorData.items) {
      ...
      SR5_SystemHelpers.srLog(
        3,
        `Parsing '${i.type}' item named '${i.name}'`,
        i
      );
      switch (i.type) {
        case "itemQuality":
        ...
        case "itemGear":
          if (!iData.isSlavedToPan)
            actorData.data.matrix.potentialPanObject.gears[i.uuid] = i.name;
          if (iData.isActive && iData.wirelessTurnedOn)
            actorData.data.matrix.connectedObject.gears[i.id] = i.name;
        ...
        case "itemArmor":
          ...
          if (iData.isActive) {
            ...
            if (!iData.isAccessory)
              SR5_EntityHelpers.updateModifier(
                actorData.data.itemsProperties.armor,
                `${i.name}`,
                modifierType,
                iData.armorValue.value
              );
            if (!iData.isAccessory)
              SR5_EntityHelpers.updateModifier(
                actorData.data.resistances.fall,
                `${i.name}`,
                modifierType,
                iData.armorValue.value
              );
          ...
          if (iData.isActive && iData.wirelessTurnedOn)
            actorData.data.matrix.connectedObject.armors[i.id] = i.name;
          if (!iData.isSlavedToPan)
            actorData.data.matrix.potentialPanObject.armors[i.uuid] = i.name;
          break;
        ...
        case "itemAugmentation":
          ...
          if (!iData.isAccessory) {
            SR5_EntityHelpers.updateModifier(
              actorData.data.essence,
              `${i.name}`,
              `${game.i18n.localize(lists.itemTypes[i.type])}`,
              -iData.essenceCost.value
            );
          }
          if (iData.isActive && Object.keys(iData.customEffects).length) {
            SR5_CharacterUtility.applyCustomEffects(i.data, actorData);
          ...
          if (iData.isActive && iData.wirelessTurnedOn)
            actorData.data.matrix.connectedObject.augmentations[i.id] = i.name;
          if (!iData.isSlavedToPan)
            actorData.data.matrix.potentialPanObject.augmentations[i.uuid] =
              i.name;
          break;

        case "itemAdeptPower":
          SR5_EntityHelpers.updateModifier(
            actorData.data.magic.powerPoints,
            i.name,
            `${game.i18n.localize(lists.itemTypes[i.type])}`,
            iData.powerPointsCost.value
          );
        ...
        case "itemDevice":
          if (actorData.type === "actorPc" || actorData.type === "actorGrunt") {
            iData.conditionMonitors.matrix.value =
              Math.ceil(iData.deviceRating / 2) + 8;
            if (iData.isActive) {
        ...
        case "itemProgram":
          if (
            ...
            if (iData.isActive)
              SR5_EntityHelpers.updateModifier(
                actorData.data.matrix.programsCurrentActive,
                `${i.name}`,
                `${game.i18n.localize(lists.itemTypes[i.type])}`,
                1
              );
          ...
          if (iData.isActive && Object.keys(iData.customEffects).length) {
            ...
            if (
              iData.type !== "autosoft" &&
              (actorData.type === "actorPc" || actorData.type === "actorGrunt")
            )
              SR5_CharacterUtility.applyCustomEffects(i.data, actorData);
          }
          break;
        ...
        case "itemNuyen":
          ...
          if (iData.amount && iData.type) {
            let resourceLabel = `${game.i18n.localize(
              lists.transactionsTypes[iData.type]
            )} (${i.name})`;
            switch (i.type) {
              case "itemKarma":
        ...
        case "itemWeapon":
          ...
          for (let mode of Object.entries(iData.firingMode)) {
            if (mode[1].value)
              modes.push(
                game.i18n.localize(SR5.weaponModesAbbreviated[mode[0]])
              );
          }
          SR5_UtilityItem._handleVisionAccessory(iData, actorData);
          if (actorData.data.matrix) {
            if (iData.isActive && iData.wirelessTurnedOn)
              actorData.data.matrix.connectedObject.weapons[i.id] = i.name;
            if (!iData.isSlavedToPan)
              actorData.data.matrix.potentialPanObject.weapons[i.uuid] = i.name;
          }
          break;
        ...
        case "itemFocus":
          ...
          switch (iData.type) {
            ...
            case "qi":
              break;
            case "sustaining":
              iData.spellChoices = SR5_UtilityItem._focusMaintien(
                iData,
                actorData
              );
              if (iData.isActive) {
                let sustainedSpell = actorData.items.find(
                  (s) => s.name == iData.sustainedSpell
                );
                if (
                  sustainedSpell &&
            ...
            default:
              SR5_SystemHelpers.srLog(
                3,
                `Unknown focus type '${iData.type}' in 'prepareEmbeddedDocuments()'`
              );
          }
          if (iData.isActive && Object.keys(iData.customEffects).length) {
            SR5_CharacterUtility.applyCustomEffects(i.data, actorData);
        ...
        case "itemDrug":
          if (
            (iData.isActive || iData.wirelessTurnedOn) &&
            Object.keys(iData.customEffects).length
          ) {
            SR5_CharacterUtility.applyCustomEffects(i.data, actorData);
          }
          break;
        ...
        case "itemEffect":
          ...
          if (iData.type === "signalJam")
            actorData.data.matrix.isJamming = true;
          break;
        ...
        default:
          SR5_SystemHelpers.srLog(
            1,
            `Unknown '${i.type}' item type in prepareEmbeddedDocuments()`
          );
      }
    }
  }
  ...
  sortLists(data) {
    SR5_SystemHelpers.srLog(
      3,
      `Sorting specific lists by order of translated terms`
    );
    if (data.skills)
      data.skills = SR5_EntityHelpers.sortByTranslatedTerm(
        data.skills,
        "skills"
      );
    if (data.skillGroups)
      data.skillGroups = SR5_EntityHelpers.sortByTranslatedTerm(
        data.skillGroups,
        "skillGroups"
      );
    if (data.matrix && data.matrix.actions)
      data.matrix.actions = SR5_EntityHelpers.sortByTranslatedTerm(
        data.matrix.actions,
        "matrixActions"
      );
  }
  ...
  updateItems(actor) {
    ...
    for (let i of actorData.items) {
      ...
      switch (i.data.type) {
            ...
            if (iData.isActive === true) {
              ...
              if (iData.type === "riggerCommandConsole") {
                if (actor.testUserPermission(game.user, 3))
                  SR5_CharacterUtility.updateControledVehicle(actorData);
              }
              if (iData.type === "cyberdeck") {
                if (actor.testUserPermission(game.user, 3))
                  SR5_CharacterUtility.updateAgent(actorData, i.data);
              }
        ...
        case "itemGear":
        case "itemAugmentation":
          if (Object.keys(iData.accessory).length)
            SR5_UtilityItem._updatePluggedAccessory(i.data, actorData);
          break;
        case "itemSpell":
        case "itemWeapon":
        case "itemKnowledge":
  ...
  async takeDamage(options) {
    ...
    if (actorData.data.specialProperties?.damageReduction)
      damageReduction = actorData.data.specialProperties.damageReduction.value;
    ...
    switch (actorData.type) {
      ...
      case "actorSpirit":
        ...
        ui.notifications.info(
          `${this.name}: ${damage}${game.i18n.localize(
            SR5.damageTypesShort[damageType]
          )} ${game.i18n.localize("SR5.Applied")}.`
        );

        if (
          actorData.data.conditionMonitors.stun.current >
          actorData.data.conditionMonitors.stun.value
        ) {
          let carriedDamage =
            actorData.data.conditionMonitors.stun.current -
            actorData.data.conditionMonitors.stun.value;
          ...
          actorData.data.conditionMonitors.stun.current =
            actorData.data.conditionMonitors.stun.value;
          ui.notifications.info(
            `${this.name}: ${carriedDamage}${game.i18n.localize(
              SR5.damageTypesShort.physical
            )} ${game.i18n.localize("SR5.Applied")}.`
          );
        }

        if (
          actorData.data.conditionMonitors.physical.current >
          actorData.data.conditionMonitors.physical.value
        ) {
          let carriedDamage =
            actorData.data.conditionMonitors.physical.current -
            actorData.data.conditionMonitors.physical.value;
      ...
      case "actorDrone":
        if (damageType === "physical") {
          ...
          if (actorData.data.controlMode === "rigging") {
            let controler = SR5_EntityHelpers.getRealActorFromID(
              actorData.data.vehicleOwner.id
            );
            let chatData = {
        ...
        if (options.damageElement === "electricity")
          options.matrixDamageValue = Math.floor(options.damageValue / 2);
        if (options.matrixDamageValue) {
          actorData.data.conditionMonitors.matrix.current +=
            options.matrixDamageValue;
          ui.notifications.info(
            `${this.name}: ${options.matrixDamageValue} ${game.i18n.localize(
              "SR5.AppliedMatrixDamage"
            )}.`
          );
        }
        break;
    ...
    switch (actorData.type) {
      ...
      case "actorSpirit":
        if (
          ...
          await this.createDeadEffect();
        else if (
          actorData.data.conditionMonitors.stun.current >=
          actorData.data.conditionMonitors.stun.value
        )
          await this.createKoEffect();
        else if (
          (damage > actorData.data.limits.physicalLimit.value + gelAmmo ||
            damage >= 10) &&
          actorData.data.conditionMonitors.stun.current <
            actorData.data.conditionMonitors.stun.value &&
          actorData.data.conditionMonitors.physical.current <
          ...
          await this.createProneEffect(damage, actorData, gelAmmo);
        break;
      case "actorGrunt":
      case "actorDrone":
        if (
          actorData.data.conditionMonitors.condition.current >=
          actorData.data.conditionMonitors.condition.value
        )
          await this.createDeadEffect();
        else if (
          damage > actorData.data.limits.physicalLimit.value + gelAmmo ||
      ...
      case "actorDevice":
        if (
          actorData.data.conditionMonitors.matrix.current >=
          actorData.data.conditionMonitors.matrix.value
        )
          await this.createDeadEffect();
        break;
    }
    ...
    if (
      options.damageElement === "electricity" &&
      actorData.type !== "actorDrone"
    ) {
    ...
    if (options.damageElement === "fire") {
      if (this.data.data.itemsProperties.armor.value <= 0)
        await this.fireDamageEffect();
      else
        await this.checkIfCatchFire(
          options.fireTreshold,
  ...
  async createProneEffect(damage, actorData, gelAmmo) {
    ...
    if (damage >= 10)
      ui.notifications.info(
        `${this.name}: ${game.i18n.format("SR5.INFO_DamageDropProneTen", {
          damage: damage,
        })}`
      );
    else if (gelAmmo < 0)
      ui.notifications.info(
        `${this.name}: ${game.i18n.format("SR5.INFO_DamageDropProneGel", {
          damage: damage,
          limit: actorData.data.limits.physicalLimit.value,
        })}`
      );
    else
      ui.notifications.info(
        `${this.name}: ${game.i18n.format("SR5.INFO_DamageDropProne", {
          damage: damage,
          limit: actorData.data.limits.physicalLimit.value,
        })}`
      );
  ...
  async createDeadEffect() {
    ...
    await this.createEmbeddedDocuments("ActiveEffect", [effect]);
    ui.notifications.info(
      `${this.name}: ${game.i18n.localize("SR5.INFO_DamageActorDead")}`
    );
  }
  ...
  async createKoEffect() {
    ...
    await this.createEmbeddedDocuments("ActiveEffect", [effect]);
    ui.notifications.info(
      `${this.name}: ${game.i18n.localize("SR5.INFO_DamageActorKo")}`
    );
  }

  //Handle Elemental Damage : Electricity
  async electricityDamageEffect() {
    let existingEffect = this.items.find(
      (item) =>
        item.type === "itemEffect" &&
        item.data.data.type === "electricityDamage"
    );
    if (existingEffect) {
      ...
      ui.notifications.info(
        `${this.name}: ${existingEffect.name} ${game.i18n.localize(
          "SR5.INFO_DurationExtendOneRound"
        )}.`
      );
    } else {
      let effect = {
        name: `${game.i18n.localize(
          "SR5.ElementalDamage"
        )} (${game.i18n.localize("SR5.ElementalDamageElectricity")})`,
        type: "itemEffect",
      ...
      ui.notifications.info(
        `${this.name}: ${effect.name} ${game.i18n.localize("SR5.Applied")}.`
      );
  ...
  async acidDamageEffect(damage, source) {
    let existingEffect = this.items.find(
      (item) =>
        item.type === "itemEffect" && item.data.data.type === "acidDamage"
    );
    let armor = this.items.find(
      (item) =>
        item.type === "itemArmor" &&
        item.data.data.isActive &&
        !item.data.data.isAccessory
    );
    if (existingEffect) {
      return;
    } else {
      if (armor) {
        ...
        let armorEffect = {
          name: `${game.i18n.localize(
            "SR5.ElementalDamage"
          )} (${game.i18n.localize("SR5.ElementalDamageAcid")})`,
        ...
        ui.notifications.info(
          `${this.name}: ${game.i18n.format("SR5.INFO_AcidReduceArmor", {
            armor: armor.name,
          })}`
        );
      }
      ...
      let effect = {
        ...
        "data.target": `${game.i18n.localize(
          "SR5.Armor"
        )}, ${game.i18n.localize("SR5.Damage")}`,
        "data.value": damage,
        "data.durationType": "round",
  ...
  async fireDamageEffect() {
    let existingEffect = this.items.find(
      (item) =>
        item.type === "itemEffect" && item.data.data.type === "fireDamage"
    );
    if (existingEffect) return;
    let effect = {
      name: `${game.i18n.localize("SR5.ElementalDamage")} (${game.i18n.localize(
        "SR5.ElementalDamageFire"
      )})`,
      type: "itemEffect",
  ...
  async rebootDeck() {
    ...
    ui.notifications.info(
      `${actorData.matrix.deviceName} ${game.i18n.localize("SR5.Rebooted")}.`
    );
  }
  ...
  static async _socketDeleteMarksOnActor(message) {
    await SR5Actor.deleteMarksOnActor(
      message.data.actorData,
  ...
  static async deleteMarkInfo(actorID, item) {
    let actor = SR5_EntityHelpers.getRealActorFromID(actorID),
      deck = actor.items.find(
        (d) => d.type === "itemDevice" && d.data.data.isActive
      ),
  ...
  static async overwatchIncrease(defenseHits, actorId) {
    ...
    if (actorData.data.matrix.overwatchScore === null)
      actorData.data.matrix.overwatchScore = 0;
    actorData.data.matrix.overwatchScore += defenseHits;
    actor.update(actorData);
    ui.notifications.info(
      `${actor.name}, ${game.i18n.localize("SR5.OverwatchScoreActual")} ${
        actorData.data.matrix.overwatchScore
      }`
    );
  }
  ...
  static async _socketOverwatchIncrease(message) {
    await SR5Actor.overwatchIncrease(
      message.data.defenseHits,
      message.data.actorId
    );
  }
  ...
  resetRecoil() {
    ...
    ui.notifications.info(
      `${this.name}: ${game.i18n.localize("SR5.CumulativeRecoilSetTo0")}.`
    );
  }
  ...
  static async createSidekick(item, userId, actorId) {
    ...
    let img;
    if (item.img === `systems/sr5/assets/img/items/${item.type}.svg`)
      img = `systems/sr5/assets/img/actors/${petType}.svg`;
    else img = item.img;
    ...
    if (item.type === "itemSpirit") {
      let baseItems = await SR5_CompendiumUtility.getBaseItems(
        "actorSpirit",
        itemData.type,
        itemData.itemRating
      );
      baseItems = await SR5_CompendiumUtility.addOptionalSpiritPowersFromItem(
        baseItems,
        itemData.optionalPowers
      );
      data = mergeObject(data, {
        ...
        "data.conditionMonitors.physical.current":
          itemData.conditionMonitors.physical.current,
        "data.conditionMonitors.stun.current":
          itemData.conditionMonitors.stun.current,
        items: baseItems,
      });
    }
    ...
    if (item.type === "itemSprite") {
      ...
      data = mergeObject(data, {
        ...
        "data.creatorId": actorId,
        "data.creatorItemId": item._id,
        "data.conditionMonitors.matrix.current":
          itemData.conditionMonitors.matrix.current,
        items: baseItems,
      });
    }
    ...
    if (item.type === "itemProgram") {
      let baseItems = [];
      let ownerDeck = ownerActor.items.find(
      ...
      data = mergeObject(data, {
        ...
        "data.conditionMonitors.matrix":
          ownerDeck.data.data.conditionMonitors.matrix,
    ...
    if (item.type === "itemVehicle") {
      ...
      data = mergeObject(data, {
        ...
        "data.attributes.acceleration.natural.base":
          itemData.attributes.acceleration,
        ...
        "data.conditionMonitors.condition.current":
          itemData.conditionMonitors.condition.current,
        "data.conditionMonitors.matrix.current":
          itemData.conditionMonitors.matrix.current,
  ...
  static async dimissSidekick(actor) {
    ...
    if (actor.type === "actorSpirit") {
      ...
      modifiedItem.data.conditionMonitors.physical.current =
        actor.data.conditionMonitors.physical.current;
      modifiedItem.data.conditionMonitors.stun.current =
        actor.data.conditionMonitors.stun.current;
    ...
    if (actor.type === "actorSprite") {
      ...
      modifiedItem.data.conditionMonitors.matrix.current =
        actor.data.conditionMonitors.matrix.current;
    ...
    if (actor.type === "actorDrone") {
      ...
      modifiedItem.data.attributes.handling =
        actor.data.attributes.handling.natural.base;
      modifiedItem.data.attributes.speed =
        actor.data.attributes.speed.natural.base;
      modifiedItem.data.attributes.acceleration =
        actor.data.attributes.acceleration.natural.base;
      modifiedItem.data.attributes.body =
        actor.data.attributes.body.natural.base;
      modifiedItem.data.attributes.armor =
        actor.data.attributes.armor.natural.base;
      modifiedItem.data.attributes.pilot =
        actor.data.attributes.pilot.natural.base;
      modifiedItem.data.attributes.sensor =
        actor.data.attributes.sensor.natural.base;
      modifiedItem.data.seating = actor.data.attributes.seating.natural.base;
      modifiedItem.data.conditionMonitors.condition.current =
        actor.data.conditionMonitors.condition.current;
      modifiedItem.data.conditionMonitors.matrix.current =
        actor.data.conditionMonitors.matrix.current;
      modifiedItem.data.isCreated = false;
      modifiedItem.img = actor.img;
  ...
  static async _socketDeleteItemFromPan(message) {
    await SR5Actor.deleteItemFromPan(
      message.data.targetItem,
      message.data.actorId,
      message.data.index
    );
  }
