...
export class SR5_CharacterUtility extends Actor {
  ...
  static resetCalculatedValues(actor) {
    ...
    for (let key of Object.keys(lists.characterInitiatives)) {
      if (data.initiatives[key]) {
        ...
        if (!data.initiatives[key].dice) {
          data.initiatives[key].dice = { value: 0, base: 1, modifiers: [] };
        }
    ...
    if (!this.findActiveInitiative(actor)) {
      switch (actor.type) {
        ...
        case "actorSpirit":
          ...
          else data.initiatives.physicalInit.isActive = true;
          break;
        case "actorDevice":
  ...
  static updateKarmas(actor) {
    ...
    SR5_EntityHelpers.updateModifier(
      actor.data.streetCred,
      `${game.i18n.localize("SR5.Karma")}`,
      `${game.i18n.localize("SR5.Karma")}`,
      Math.round(actor.data.karma.value / 10),
      false,
      false
    );
  }
  ...
  static updateNotoriety(actor) {
    if (actor.data.specialProperties.notoriety.value)
      SR5_EntityHelpers.updateModifier(
        actor.data.notoriety,
        `${game.i18n.localize("SR5.ReputationNotoriety")}`,
        `${game.i18n.localize("SR5.Qualities")}`,
        actor.data.specialProperties.notoriety.value
      );
  ...
  static updatePenalties(actor) {
    if (!actor) {
      SR5_SystemHelpers.srLog(
        1,
        `Missing or invalid actor in call to 'updatePenalties()'`
      );
      return;
    }
    if (!actor.data.penalties) {
      SR5_SystemHelpers.srLog(
        1,
        `No penalties properties for '${actor.name}' actor in call to 'updatePenalties()'`
      );
      return;
    }
      ...
      switch (key) {
        ...
        case "condition":
          if (data.conditionMonitors[key]) {
            ...
            data.penalties[key].actual.base = -Math.floor(
              (data.conditionMonitors[key].current -
                data.penalties[key].boxReduction.value) /
                data.penalties[key].step.value
            );
            if (data.penalties[key].actual.base > 0)
        ...
        default:
          SR5_SystemHelpers.srLog(
            1,
            `Unknown '${key}' penalty type in 'updatePenalties()'`
          );
      }

      SR5_EntityHelpers.updateValue(data.penalties[key].actual);
      // Assure penalty is not a positive number
      if (data.penalties[key].actual.value > 0)
        data.penalties[key].actual.value = 0;
    }

    if (actor.type === "actorPc" || actor.type === "actorSpirit") {
      data.penalties.condition.actual.base =
        data.penalties.physical.actual.base + data.penalties.stun.actual.base;
      SR5_EntityHelpers.updateValue(data.penalties.condition.actual);
    }
  }

  static applyPenalty(penalty, property, actor) {
    if (!penalty || !property || !actor) {
      SR5_SystemHelpers.srLog(
        1,
        `Missing or invalid parameter in call to 'applyPenalty()'`
      );
      return;
    }
    if (!actor.data.penalties) {
      SR5_SystemHelpers.srLog(
        3,
        `No existing penalties on '${actor.name}' actor in call to 'applyPenalty()'`
      );
      return;
    }
    let lists = actor.lists,
    ...
    switch (penalty) {
      ...
      case "special":
        if (data.penalties[penalty].actual.value) {
          ...
          SR5_EntityHelpers.updateModifier(
            property,
            `${game.i18n.localize("SR5.Penalty")}`,
            type,
            data.penalties[penalty].actual.value
          );
        }
        break;
      default:
        SR5_SystemHelpers.srLog(
          3,
          `Unknown penalty type '${penalty}' in 'applyPenalty()'`
        );
        return;
    }
  }
  ...
  static handleSustaining(actor, itemType, concentrationType) {
    ...
    for (let i of actor.items) {
      if (i.data.data.systemEffects) {
        for (let is of Object.values(i.data.data.systemEffects)) {
          if (is.value === "sustainingMod1" && i.data.data.isActive)
            sustainedMod = 1;
        }
      }
    }
    ...
    for (let i of actor.items) {
      if (
        i.data.data.isActive &&
        i.type === itemType &&
        !i.data.data.freeSustain
      ) {
        SR5_EntityHelpers.updateModifier(
          actor.data.penalties[concentrationType].actual,
          `${i.name}`,
          `${game.i18n.localize(lists.itemTypes[i.type])}`,
          -sustainedMod
        );
      }
      ...
      if (
        ...
        i.type === itemType &&
        !i.data.data.freeSustain &&
        !actor.data[concentrationType].concentration &&
        (i.data.data.force <=
          actor.data.specialProperties.concentration.value ||
          i.data.data.level <= actor.data.specialProperties.concentration.value)
      ) {
        SR5_EntityHelpers.updateModifier(
          actor.data.penalties[concentrationType].actual,
          `${game.i18n.localize("SR5.QualityTypePositive")}`,
          `${game.i18n.localize("SR5.Concentration")}`,
          sustainedMod
        );
  ...
  static async handleVision(actor) {
    ...
    if (data.visions.astral.natural || data.visions.augmented)
      data.visions.astral.hasVision = true;
    if (data.visions.astral.isActive) data.visions.astral.hasVision = true;

    if (data.visions.astral.isActive) {
      SR5_EntityHelpers.updateModifier(
        data.itemsProperties.environmentalMod.visibility,
        `${game.i18n.localize("SR5.AstralPerception")}`,
        `${game.i18n.localize("SR5.VisionType")}`,
        -4,
        false,
        false
      );
      SR5_EntityHelpers.updateModifier(
        data.itemsProperties.environmentalMod.light,
        `${game.i18n.localize("SR5.AstralPerception")}`,
        `${game.i18n.localize("SR5.VisionType")}`,
        -4,
        false,
      ...
      SR5_EntityHelpers.updateModifier(
        data.itemsProperties.environmentalMod.glare,
        `${game.i18n.localize("SR5.AstralPerception")}`,
        `${game.i18n.localize("SR5.VisionType")}`,
        -4,
        false,
      ...
      SR5_EntityHelpers.updateModifier(
        data.itemsProperties.environmentalMod.wind,
        `${game.i18n.localize("SR5.AstralPerception")}`,
        `${game.i18n.localize("SR5.VisionType")}`,
        -4,
        false,
    ...
    if (data.visions.lowLight.natural || data.visions.lowLight.augmented) {
      ...
      if (data.visions.lowLight.isActive) {
        SR5_EntityHelpers.updateModifier(
          data.itemsProperties.environmentalMod.light,
          `${game.i18n.localize("SR5.LowLightVision")}`,
          `${game.i18n.localize("SR5.VisionType")}`,
          -2,
          false,
          false
    ...
    if (
      data.visions.thermographic.natural ||
      data.visions.thermographic.augmented
    ) {
      data.visions.thermographic.hasVision = true;
      if (data.visions.thermographic.isActive) {
        SR5_EntityHelpers.updateModifier(
          data.itemsProperties.environmentalMod.light,
          `${game.i18n.localize("SR5.ThermographicVision")}`,
          `${game.i18n.localize("SR5.VisionType")}`,
          -1,
          false,
          false
    ...
    if (data.visions.ultrasound.natural || data.visions.ultrasound.augmented) {
      ...
      if (data.visions.ultrasound.isActive) {
        ...
        SR5_EntityHelpers.updateModifier(
          data.itemsProperties.environmentalMod.light,
          `${game.i18n.localize("SR5.UltrasoundVision")}`,
          `${game.i18n.localize("SR5.VisionType")}`,
          -3,
          false,
          false
    ...
    if (data.itemsProperties?.environmentalMod) {
      for (let key of Object.keys(lists.environmentalModifiers)) {
        SR5_EntityHelpers.updateValue(
          data.itemsProperties.environmentalMod[key]
        );
      }
    }
  }
  ...
  static async switchVision(actor, vision) {
    ...
    for (let key of Object.keys(lists.visionActive)) {
      if (key === vision && key === currentVision)
        data.visions[key].isActive = false;
  ...
  static applyRacialModifers(actor) {
    ...
    let label = `${game.i18n.localize(
      lists.metatypes[data.biography.characterMetatype]
    )}`;
    ...
    switch (data.biography.characterMetatype) {
      ...
      case "elf":
        ...
        if (actor.type === "actorGrunt") {
          SR5_EntityHelpers.updateModifier(
            data.attributes.agility.natural,
            label,
            `${game.i18n.localize("SR5.Metatype")}`,
            1
          );
          SR5_EntityHelpers.updateModifier(
            data.attributes.charisma.natural,
            label,
            `${game.i18n.localize("SR5.Metatype")}`,
            2
          );
      ...
      case "dwarf":
        ...
        for (let vector of Object.keys(lists.propagationVectors)) {
          SR5_EntityHelpers.updateModifier(
            data.resistances.disease[vector],
            label,
            `${game.i18n.localize("SR5.Metatype")}`,
            2
          );
          SR5_EntityHelpers.updateModifier(
            data.resistances.toxin[vector],
            label,
            `${game.i18n.localize("SR5.Metatype")}`,
            2
          );
        ...
        if (actor.type === "actorGrunt") {
          SR5_EntityHelpers.updateModifier(
            data.attributes.body.natural,
            label,
            `${game.i18n.localize("SR5.Metatype")}`,
            2
          );
          SR5_EntityHelpers.updateModifier(
            data.attributes.reaction.natural,
            label,
            `${game.i18n.localize("SR5.Metatype")}`,
            -1
          );
          SR5_EntityHelpers.updateModifier(
            data.attributes.strength.natural,
            label,
            `${game.i18n.localize("SR5.Metatype")}`,
            2
          );
          SR5_EntityHelpers.updateModifier(
            data.attributes.willpower.natural,
            label,
            `${game.i18n.localize("SR5.Metatype")}`,
            1
          );
      ...
      case "troll":
        ...
        SR5_EntityHelpers.updateModifier(
          data.resistances.physicalDamage,
          label,
          `${game.i18n.localize("SR5.Metatype")}`,
          1
        );
      ...
      default:
        SR5_SystemHelpers.srLog(
          3,
          `Unknown metatype '${data.biography.characterMetatype}' in 'applyRacialModifers()'`
        );
        return;
    }
  }
  ...
  static updateSpiritAttributes(actor) {
    ...
    switch (data.type) {
      case "watcher":
        ...
        SR5_EntityHelpers.updateModifier(
          attributes.willpower.natural,
          label,
          "spiritType",
          -2
        );
        SR5_EntityHelpers.updateModifier(
          attributes.logic.natural,
          label,
          "spiritType",
          -2
        );
        SR5_EntityHelpers.updateModifier(
          attributes.intuition.natural,
          label,
          "spiritType",
          -2
        );
        SR5_EntityHelpers.updateModifier(
          attributes.charisma.natural,
          label,
          "spiritType",
          -2
        );
      ...
      case "homunculus":
        attributes.body.natural.base = 0;
        SR5_EntityHelpers.updateModifier(
          attributes.agility.natural,
          label,
          "spiritType",
          -2
        );
        SR5_EntityHelpers.updateModifier(
          attributes.reaction.natural,
          label,
          "spiritType",
          -2
        );
      ...
      default:
        SR5_SystemHelpers.srLog(
          3,
          `Unknown ${data.type} spirit type in 'updateSpiritAttributes()'`
        );
        return false;
    }
  }
  ...
  static updateSpriteValues(actor) {
    let data = actor.data,
      attributes = data.attributes,
      specialAttributes = data.specialAttributes,
      matrixAttributes = data.matrix.attributes;
    let lists = actor.lists;
  ...
  static updateSpecialAttributes(actor) {
    ...
    for (let key of Object.keys(lists.characterSpecialAttributes)) {
      if (data.specialAttributes[key]) {
        ...
        data.specialAttributes[key].augmented.base =
          data.specialAttributes[key].natural.value;
        if (key == "magic" || key == "resonance") {
          if (data.essence?.base - data.essence?.value) {
            SR5_EntityHelpers.updateModifier(
              data.specialAttributes[key].augmented,
              game.i18n.localize("SR5.EssenceLoss"),
              "",
              -1 * Math.ceil(data.essence.base - data.essence.value)
            );
          }
        }
    ...
    if (actor.type === "actorPc" || actor.type === "actorGrunt") {
      ...
      // Update encumbrance
      let armorAccessoriesModifiers =
        data.itemsProperties.armor.modifiers.filter(
          (m) => m.type == "armorAccessory"
        );
      if (armorAccessoriesModifiers) {
        let totalArmorAccessoriesValue = SR5_EntityHelpers.modifiersSum(
          armorAccessoriesModifiers
        );
        if (
          totalArmorAccessoriesValue >
          data.attributes.strength.augmented.value + 1
        ) {
          let armorPenalty = Math.floor(
            (totalArmorAccessoriesValue -
              data.attributes.strength.augmented.value) /
              2
          );
          SR5_EntityHelpers.updateModifier(
            data.attributes.agility.augmented,
            game.i18n.localize("SR5.ArmorEncumbrance"),
            "armorEncumbrance",
            -1 * armorPenalty
          );
          SR5_EntityHelpers.updateModifier(
            data.attributes.reaction.augmented,
            game.i18n.localize("SR5.ArmorEncumbrance"),
            "armorEncumbrance",
            -1 * armorPenalty
          );
  ...
  static updateDerivedAttributes(actor) {
    ...
    for (let key of Object.keys(lists.characterDerivedAttributes)) {
      ...
      switch (key) {
        case "composure":
          SR5_EntityHelpers.updateModifier(
            derivedAttributes[key],
            `${game.i18n.localize("SR5.Charisma")}`,
            `${game.i18n.localize("SR5.LinkedAttribute")}`,
            attributes.charisma.augmented.value
          );
          SR5_EntityHelpers.updateModifier(
            derivedAttributes[key],
            `${game.i18n.localize("SR5.Willpower")}`,
            `${game.i18n.localize("SR5.LinkedAttribute")}`,
            attributes.willpower.augmented.value
          );
          break;
        case "judgeIntentions":
          ...
          SR5_EntityHelpers.updateModifier(
            derivedAttributes[key],
            `${game.i18n.localize("SR5.Intuition")}`,
            `${game.i18n.localize("SR5.LinkedAttribute")}`,
            attributes.intuition.augmented.value
          );
          break;
        case "memory":
          SR5_EntityHelpers.updateModifier(
            derivedAttributes[key],
            `${game.i18n.localize("SR5.Logic")}`,
            `${game.i18n.localize("SR5.LinkedAttribute")}`,
            attributes.logic.augmented.value
          );
          SR5_EntityHelpers.updateModifier(
            derivedAttributes[key],
            `${game.i18n.localize("SR5.Willpower")}`,
        ...
        case "surprise":
          SR5_EntityHelpers.updateModifier(
            derivedAttributes[key],
            `${game.i18n.localize("SR5.Reaction")}`,
            `${game.i18n.localize("SR5.LinkedAttribute")}`,
            attributes.reaction.augmented.value
          );
          SR5_EntityHelpers.updateModifier(
            derivedAttributes[key],
            `${game.i18n.localize("SR5.Intuition")}`,
        ...
        default:
          SR5_SystemHelpers.srLog(
            1,
            `Unknown derived attribute '${key}' in 'updateDerivedAttributes()'`
          );
      }
  ...
  static updateRecoil(actor) {
    if (actor.type === "actorDrone")
      actor.data.recoilCompensation.base =
        actor.data.attributes.body.augmented.value;
    else
      actor.data.recoilCompensation.base = parseInt(
        1 + Math.ceil(actor.data.attributes.strength.augmented.value / 3)
      );
  ...
  static updateEncumbrance(actor) {
    ...
    for (let key of Object.keys(lists.weightActions)) {
      switch (key) {
        case "carry":
          weightActions[key].baseWeight.base =
            attributes.strength.augmented.value * 10;
          weightActions[key].extraWeight.base = 10;
          break;
        case "lift":
          weightActions[key].baseWeight.base =
            attributes.strength.augmented.value * 15;
          weightActions[key].extraWeight.base = 15;
          break;
        case "liftAboveHead":
          weightActions[key].baseWeight.base =
            attributes.strength.augmented.value * 5;
          weightActions[key].extraWeight.base = 5;
          break;
        default:
          SR5_SystemHelpers.srLog(
            1,
            `Unknown weights action '${key}' in 'updateEncumbrance()'`
          );
      }
      ...
      SR5_EntityHelpers.updateModifier(
        weightActions[key].test,
        `${game.i18n.localize("SR5.Strength")}`,
        `${game.i18n.localize("SR5.LinkedAttribute")}`,
        attributes.strength.augmented.value
      );
      SR5_EntityHelpers.updateModifier(
        weightActions[key].test,
        `${game.i18n.localize("SR5.Body")}`,
        `${game.i18n.localize("SR5.LinkedAttribute")}`,
        attributes.body.augmented.value
      );
  ...
  static updateMovements(actor) {
    ...
    for (let key of Object.keys(lists.movements)) {
      ...
      switch (key) {
        ...
        case "verticalJump":
          let height =
            biography && biography.characterHeight
              ? biography.characterHeight / 100
              : 1;
          SR5_EntityHelpers.updateModifier(
            movements[key].test,
            `${game.i18n.localize("SR5.Agility")}`,
            `${game.i18n.localize("SR5.LinkedAttribute")}`,
            attributes.agility.augmented.value
          );
          SR5_EntityHelpers.updateModifier(
            movements[key].test,
            `${game.i18n.localize("SR5.SkillGymnastics")}`,
            `${game.i18n.localize("SR5.Skill")}`,
            skills.gymnastics.rating.value
          );
        ...
        case "horizontalJumpStanding":
          ...
          movements[key].maximum.base = Math.ceil(
            1.5 * attributes.agility.augmented.value
          );
          break;
        ...
        case "holdBreath":
          ...
          SR5_EntityHelpers.updateModifier(
            movements[key].test,
            `${game.i18n.localize("SR5.SkillSwimming")}`,
            `${game.i18n.localize("SR5.Skill")}`,
            skills.swimming.rating.value
          );
        ...
        case "run":
          ...
          SR5_EntityHelpers.updateModifier(
            movements[key].test,
            `${game.i18n.localize("SR5.SkillRunning")}`,
            `${game.i18n.localize("SR5.Skill")}`,
            skills.running.rating.value
          );
          movements[key].movement.base = attributes.agility.augmented.value * 4;
          if (
            biography &&
            (biography.characterMetatype === "dwarf" ||
              biography.characterMetatype === "troll")
          )
            movements[key].extraMovement.base = 1;
          else {
            if (actor.type == "actorSpirit") {
        ...
        case "swim":
          ...
          movements[key].movement.base = Math.ceil(
            (attributes.strength.augmented.value +
              attributes.agility.augmented.value) /
              2
          );
          if (
            biography &&
            (biography.characterMetatype === "elf" ||
              biography.characterMetatype === "troll")
          )
            movements[key].extraMovement.base = 2;
          else {
        ...
        case "treadWater":
          ...
          movements[key].extraMovement.base =
            attributes.strength.augmented.value;
          break;
        case "walk":
          SR5_EntityHelpers.updateModifier(
            ...
            `${game.i18n.localize("SR5.Agility")}`,
            `${game.i18n.localize("SR5.LinkedAttribute")}`,
            attributes.agility.augmented.value * 2
          );
          break;
        default:
          SR5_SystemHelpers.srLog(
            1,
            `Unknown movement '${key}' in 'updateMovements()'`
          );
      }
  ...
  static updateConditionMonitors(actor) {
    let lists = actor.lists,
      data = actor.data,
      conditionMonitors = data.conditionMonitors,
      attributes = data.attributes,
      specialAttributes = data.specialAttributes;
    if (actor.type == "actorSpirit") {
    ...
    for (let key of Object.keys(lists.monitorTypes)) {
      if (conditionMonitors[key]) {
        switch (key) {
          case "stun":
            conditionMonitors[key].base = Math.ceil(
              attributes.willpower.augmented.value / 2 + 8
            );
            break;
          case "physical":
            conditionMonitors[key].base = Math.ceil(
              attributes.body.augmented.value / 2 + 8
            );
            break;
          case "condition":
            if (actor.type == "actorDrone") {
              if (data.type === "drone")
                conditionMonitors[key].base = Math.ceil(
                  attributes.body.augmented.value / 2 + 6
                );
              else
                conditionMonitors[key].base = Math.ceil(
                  attributes.body.augmented.value / 2 + 12
                );
            } else {
              conditionMonitors[key].base = Math.max(
                Math.ceil(attributes.willpower.augmented.value / 2 + 8),
                Math.ceil(attributes.body.augmented.value / 2 + 8)
              );
            }
            break;
          case "overflow":
          ...
          case "matrix":
            conditionMonitors[key].base = Math.ceil(
              data.matrix.deviceRating / 2 + 8
            );
            break;
          case "edge":
            conditionMonitors[key].base =
              specialAttributes.edge.augmented.value;
            break;
          default:
            SR5_SystemHelpers.srLog(
              1,
              `Unknown '${key}' condition monitor type in 'updateConditionMonitors()'`
            );
            return;
        }
  ...
  static updateInitiativePhysical(actor) {
    let data = actor.data,
      ...
      attributes = data.attributes,
      initPhy = initiatives.physicalInit,
      lists = actor.lists;
      ...
      case "actorDrone":
        ...
        switch (data.controlMode) {
          case "autopilot":
            SR5_EntityHelpers.updateModifier(
              initPhy,
              `${game.i18n.localize("SR5.VehicleStat_PilotShort")}`,
              `${game.i18n.localize("SR5.LinkedAttribute")}`,
              attributes.pilot.augmented.value
            );
            SR5_EntityHelpers.updateModifier(
              initPhy,
              `${game.i18n.localize("SR5.VehicleStat_PilotShort")}`,
          ...
          case "manual":
            SR5_EntityHelpers.updateModifier(
              initPhy,
              `${game.i18n.localize("SR5.InitiativePhysical")}`,
              `${game.i18n.localize("SR5.Controler")}`,
              controlerData.initiatives.physicalInit.value
            );
            SR5_EntityHelpers.updateModifier(
              initPhy.dice,
              `${game.i18n.localize("SR5.InitiativePhysical")}`,
              `${game.i18n.localize("SR5.Controler")}`,
              controlerData.initiatives.physicalInit.dice.value
            );
            break;
          case "remote":
          case "rigging":
            SR5_EntityHelpers.updateModifier(
              initPhy,
              `${game.i18n.localize("SR5.InitiativeMatrix")}`,
              `${game.i18n.localize("SR5.Controler")}`,
              controlerData.initiatives.matrixInit.value
            );
            SR5_EntityHelpers.updateModifier(
              initPhy.dice,
              `${game.i18n.localize("SR5.InitiativeMatrix")}`,
              `${game.i18n.localize("SR5.Controler")}`,
              controlerData.initiatives.matrixInit.dice.value
            );
            break;
          default:
            SR5_SystemHelpers.srLog(
              1,
              `Unknown controle mode '${data.controlMode}' in 'updateInitiatives() for drone/vehicle'`
            );
        }
        break;
      case "actorSpirit":
        ...
        initPhy.dice.base = 1;
        if (data.type !== "homunculus")
          SR5_EntityHelpers.updateModifier(
            initPhy.dice,
            `${game.i18n.localize("SR5.SpiritType")}`,
            `${game.i18n.localize(lists.spiritTypes[data.type])}`,
            1
          );
        break;
      default:
  ...
  static updateInitiativeAstral(actor) {
    let data = actor.data,
      initiatives = data.initiatives,
      attributes = data.attributes,
      initAst = initiatives.astralInit,
      lists = actor.lists;
    ...
    if (actor.type === "actorSpirit") {
      SR5_EntityHelpers.updateModifier(
        initAst,
        `${game.i18n.localize("SR5.SpiritForce")}`,
        `${game.i18n.localize("SR5.LinkedAttribute")}`,
        data.force.value
      );
      switch (data.type) {
      ...
      SR5_EntityHelpers.updateModifier(
        initAst.dice,
        `${game.i18n.localize("SR5.AstralProjection")}`,
        `${game.i18n.localize("SR5.AstralPlane")}`,
        3
      );
    }
  ...
  static updateInitiativeMatrix(actor) {
    let data = actor.data,
      initiatives = data.initiatives,
      attributes = data.attributes,
      initMat = initiatives.matrixInit,
      matrixAttributes = data.matrix.attributes,
      lists = actor.lists;
    initMat.base = 0;
    initMat.dice.base = 0;
    ...
    switch (actor.type) {
      ...
      case "actorGrunt":
        switch (data.matrix.userMode) {
          case "ar":
            initMat.modifiers = initMat.modifiers.concat(
              initiatives.physicalInit.modifiers
            );
            initMat.dice.base = 1;
            initMat.dice.modifiers = initMat.dice.modifiers.concat(
              initiatives.physicalInit.dice.modifiers
            );
            break;
          case "coldsim":
            SR5_EntityHelpers.updateModifier(
            ...
            SR5_EntityHelpers.updateModifier(
              initMat,
              data.matrix.deviceName,
              `${game.i18n.localize("SR5.Device")}`,
              matrixAttributes.dataProcessing.value
            );
            SR5_EntityHelpers.updateModifier(
              initMat.dice,
              game.i18n.localize("SR5.VirtualRealityColdSimShort"),
              game.i18n.localize("SR5.MatrixUserMode"),
              3
            );
            break;
          case "hotsim":
          ...
          default:
            SR5_SystemHelpers.srLog(
              1,
              `Unknown matrix userMode '${data.matrix.userMode}' in 'updateInitiativeMatrix()'`
            );
        }
        break;
      case "actorSprite":
        SR5_EntityHelpers.updateModifier(
          initMat,
          `${game.i18n.localize("SR5.Level")}`,
          `${game.i18n.localize("SR5.LinkedAttribute")}`,
          data.level
        );
        SR5_EntityHelpers.updateModifier(
          initMat,
          `${game.i18n.localize("SR5.DataProcessing")}`,
          `${game.i18n.localize("SR5.LinkedAttribute")}`,
          matrixAttributes.dataProcessing.value
        );
        SR5_EntityHelpers.updateModifier(
          initMat.dice,
          `${game.i18n.localize(lists.spriteTypes[data.type])}`,
          `${game.i18n.localize("ACTOR.TypeActorsprite")}`,
          4
        );
        break;
      case "actorAgent":
        ...
        SR5_EntityHelpers.updateModifier(
          initMat.dice,
          `${game.i18n.localize(lists.spriteTypes[data.type])}`,
          `${game.i18n.localize("ACTOR.TypeActoragent")}`,
          4
        );
        break;
      case "actorDevice":
        ...
        SR5_EntityHelpers.updateModifier(
          initMat,
          `${game.i18n.localize("SR5.DeviceRating")}`,
          `${game.i18n.localize("SR5.LinkedAttribute")}`,
          data.matrix.deviceRating
        );
        SR5_EntityHelpers.updateModifier(
          initMat,
          `${game.i18n.localize("SR5.DataProcessing")}`,
        ...
        SR5_EntityHelpers.updateModifier(
          initMat.dice,
          `${game.i18n.localize(lists.deviceTypes[data.matrix.deviceType])}`,
          `${game.i18n.localize("ACTOR.TypeActordevice")}`,
          4
        );
        break;
      default:
        SR5_SystemHelpers.srLog(
          1,
          `Unknown actor type '${actor.type}' in 'updateInitiativeMatrix()'`
        );
    }

    if (data.matrix.userMode !== "ar")
      this.applyPenalty("condition", initMat, actor);
  ...
  static async switchToInitiative(entity, initiative) {
    ...
    if (currentInitiative === "astralInit")
      data.visions.astral.isActive = false;
    ...
    let previousInitiativeEffect = actor.data.effects.find(
      (effect) => effect.data.origin === "initiativeMode"
    );
    //generate effect
    let initiativeEffect;
    if (initiative !== "physicalInit")
      initiativeEffect =
        SR5_CharacterUtility.generateInitiativeEffect(initiative);

    // if initiative is physical remove effect, else add or update active effect
    if (initiative === "physicalInit") {
      if (previousInitiativeEffect)
        await actor.deleteEmbeddedDocuments("ActiveEffect", [
          previousInitiativeEffect.id,
        ]);
    } else {
      if (previousInitiativeEffect)
        await previousInitiativeEffect.update(initiativeEffect);
  ...
  static generateInitiativeEffect(initiativeType) {
    ...
    switch (initiativeType) {
      case "astralInit":
        ...
        initiativeIcon = "systems/sr5/assets/img/status/StatusInitAstalOn.svg";
        break;
      case "matrixInit":
        initiativeLabel = game.i18n.localize("SR5.InitiativeMatrix");
        initiativeIcon = "systems/sr5/assets/img/status/StatusInitMatrixOn.svg";
        break;
      default:
        SR5_SystemHelpers.srLog(
          1,
          `Unknown initiative mode '${initiative}' in 'switchToInitiative()'`
        );
    }
      ...
      if (defenses[key]) {
        ...
        switch (key) {
          case "block":
            ...
            SR5_EntityHelpers.updateModifier(
              defenses[key],
              `${game.i18n.localize("SR5.SkillUnarmedCombat")}`,
              `${game.i18n.localize("SR5.Skill")}`,
              skills.unarmedCombat.rating.value
            );
            break;
          case "defend":
            if (actor.type == "actorDrone") {
              ...
              switch (data.controlMode) {
                ...
                case "manual":
                  SR5_EntityHelpers.updateModifier(
                    ...
                    `${game.i18n.localize("SR5.Reaction")}`,
                    `${game.i18n.localize("SR5.Controler")}`,
                    controlerData.attributes.reaction.augmented.value
                  );
                  SR5_EntityHelpers.updateModifier(
                    defenses[key],
                    `${game.i18n.localize("SR5.Intuition")}`,
                    `${game.i18n.localize("SR5.Controler")}`,
                    controlerData.attributes.intuition.augmented.value
                  );
                  break;
                case "rigging":
                  SR5_EntityHelpers.updateModifier(
                  ...
                  if (controlerData.specialProperties.controlRig.value)
                    SR5_EntityHelpers.updateModifier(
                      defenses[key],
                      `${game.i18n.localize("SR5.ControlRig")}`,
                      `${game.i18n.localize("SR5.Augmentation")}`,
                      controlerData.specialProperties.controlRig.value
                    );
                  if (controlerData.matrix.userMode === "hotsim")
                    SR5_EntityHelpers.updateModifier(
                      defenses[key],
                      `${game.i18n.localize("SR5.VirtualRealityHotSimShort")}`,
                      `${game.i18n.localize("SR5.MatrixUserMode")}`,
                      1
                    );
                  break;
                default:
                  SR5_SystemHelpers.srLog(
                    1,
                    `Unknown controle mode '${data.controlMode}' in 'updateDefenses() for drone/vehicle'`
                  );
              }
            } else {
              SR5_EntityHelpers.updateModifier(
          ...
          case "parryClubs":
            ...
            SR5_EntityHelpers.updateModifier(
              defenses[key],
              `${game.i18n.localize("SR5.SkillClubs")}`,
              `${game.i18n.localize("SR5.Skill")}`,
              skills.clubs.rating.value
            );
            break;
          case "parryBlades":
            SR5_EntityHelpers.updateModifier(
            ...
            SR5_EntityHelpers.updateModifier(
              defenses[key],
              `${game.i18n.localize("SR5.SkillBlades")}`,
              `${game.i18n.localize("SR5.Skill")}`,
              skills.blades.rating.value
            );
            break;
          default:
            SR5_SystemHelpers.srLog(
              1,
              `Unknown '${key}' defense in 'updateDefenses()'`
            );
        }
  ...
  static updateArmor(actor) {
    ...
    for (let key of Object.keys(lists.specialDamageTypes)) {
      SR5_EntityHelpers.updateValue(
        actor.data.itemsProperties.armor.specialDamage[key],
        0
      );
    }
  }
      ...
      if (resistances[key]) {
        switch (key) {
          ...
          case "specialDamage":
            for (let specialDamage of Object.keys(lists.specialDamageTypes)) {
              if (actor.type == "actorDrone") {
                ...
                SR5_EntityHelpers.updateModifier(
                  resistances[key][specialDamage],
                  `${game.i18n.localize("SR5.VehicleStat_ArmorShort")}`,
                  `${game.i18n.localize("SR5.LinkedAttribute")}`,
                  attributes.armor.augmented.value
                );
              } else {
                resistances[key][specialDamage].base = 0;
                SR5_EntityHelpers.updateModifier(
                ...
                if (data.itemsProperties) {
                  resistances.specialDamage[specialDamage].modifiers =
                    resistances.specialDamage[specialDamage].modifiers.concat(
                      data.itemsProperties.armor.modifiers
                    );
                  resistances.specialDamage[specialDamage].modifiers =
                    resistances.specialDamage[specialDamage].modifiers.concat(
                      data.itemsProperties.armor.specialDamage[specialDamage]
                        .modifiers
                    );
                }
              }
              SR5_EntityHelpers.updateDicePool(
                resistances[key][specialDamage],
                0
              );
            }
          ...
          case "fall":
            ...
            if (data.itemsProperties)
              resistances[key].modifiers = resistances[key].modifiers.concat(
                data.itemsProperties.armor.modifiers
              );
            SR5_EntityHelpers.updateDicePool(resistances[key], 0);
            break;
          ...
          default:
            SR5_SystemHelpers.srLog(
              1,
              `Unknown resistance '${key}' in 'updateResistances()'`
            );
        }
      }
    }
    ...
    for (let key of Object.keys(lists.characterLimits)) {
      switch (key) {
        case "astralLimit":
          if (limits[key]) {
            limits[key].base = Math.max(
              Math.ceil(
                (attributes.logic.augmented.value * 2 +
                  attributes.intuition.augmented.value +
                  attributes.willpower.augmented.value) /
                  3
              ),
              Math.ceil(
                (attributes.charisma.augmented.value * 2 +
                  attributes.willpower.augmented.value +
                  data.essence.value) /
                  3
              )
        ...
        case "physicalLimit":
          if (limits[key]) {
            limits[key].base = Math.ceil(
              (attributes.strength.augmented.value * 2 +
                attributes.body.augmented.value +
                attributes.reaction.augmented.value) /
                3
            );
        ...
        default:
          SR5_SystemHelpers.srLog(
            1,
            `Unknown limit '${key}' in 'updateLimits()'`
          );
      }
  ...
  static generateVehicleSkills(actor) {
    ...
    switch (data.controlMode) {
      case "autopilot":
        SR5_EntityHelpers.updateModifier(
          skills.sneaking.rating,
          game.i18n.localize("SR5.VehicleStat_PilotShort"),
          game.i18n.localize("SR5.LinkedAttribute"),
          attributes.pilot.augmented.value
        );
        SR5_EntityHelpers.updateModifier(
          skills.sneaking.limit,
          game.i18n.localize("SR5.VehicleStat_HandlingShort"),
          game.i18n.localize("SR5.LinkedAttribute"),
          attributes.handling.augmented.value
        );
        SR5_EntityHelpers.updateModifier(
          skills.perception.rating,
          game.i18n.localize("SR5.VehicleStat_PilotShort"),
          game.i18n.localize("SR5.LinkedAttribute"),
          attributes.pilot.augmented.value
        );
        SR5_EntityHelpers.updateModifier(
          skills.perception.limit,
          game.i18n.localize("SR5.VehicleStat_SensorShort"),
          game.i18n.localize("SR5.LinkedAttribute"),
          attributes.sensor.augmented.value
        );
        break;
      case "remote":
        SR5_EntityHelpers.updateModifier(
          skills.perception.test,
          game.i18n.localize("SR5.ControlMode"),
          game.i18n.localize("SR5.ControlMode"),
          controlerData.skills.perception.test.dicePool
        );
        SR5_EntityHelpers.updateModifier(
          skills.perception.limit,
          game.i18n.localize("SR5.VehicleStat_SensorShort"),
        ...
        if (
          controlerData.matrix.attributes.dataProcessing.value <
          attributes.sensor.augmented.value
        ) {
          let mod =
            controlerData.matrix.attributes.dataProcessing.value -
          ...
          SR5_EntityHelpers.updateModifier(
            skills.perception.limit,
            game.i18n.localize("SR5.DataProcessingLimit"),
            game.i18n.localize("SR5.ControlMode"),
            mod
          );
        }
        ...
        if (data.pilotSkill) {
          SR5_EntityHelpers.updateModifier(
            skills.sneaking.test,
            game.i18n.localize("SR5.Controler"),
            game.i18n.localize("SR5.ControlMode"),
            controlerData.skills[data.pilotSkill].test.dicePool
          );
        }
        SR5_EntityHelpers.updateModifier(
          skills.sneaking.limit,
      ...
      case "rigging":
        ...
        SR5_EntityHelpers.updateModifier(
          skills.sneaking.limit,
          game.i18n.localize("SR5.ControlRigging"),
          game.i18n.localize("SR5.ControlMode"),
          1
        );
        if (controlerData.matrix.userMode === "hotsim") {
      ...
      default:
        SR5_SystemHelpers.srLog(
          1,
          `Unknown controle mode '${data.controlMode}' in 'generateVehicleSkills()'`
        );
    }
    ...
    skills.sneaking.test.modifiers = skills.sneaking.test.modifiers.concat(
      skills.sneaking.rating.modifiers
    );
    skills.perception.test.base = 0;
    skills.perception.test.modifiers = skills.perception.test.modifiers.concat(
      skills.perception.rating.modifiers
    );
    ...
    switch (data.controlMode) {
      ...
      case "remote":
        if (data.pilotSkill)
          SR5_EntityHelpers.updateModifier(
            vehicleTest.test,
            `${game.i18n.localize("SR5.Controler")} (${game.i18n.localize(
              lists.pilotSkills[data.pilotSkill]
            )})`,
            game.i18n.localize("SR5.ControlMode"),
            controlerData.skills[data.pilotSkill].test.dicePool
          );
        if (controlerData.matrix.userMode === "ar")
          SR5_EntityHelpers.updateModifier(
            vehicleTest.limit,
            game.i18n.localize("SR5.AugmentedReality"),
            game.i18n.localize("SR5.MatrixUserMode"),
            1
          );
        else if (
          controlerData.matrix.userMode === "coldsim" ||
          controlerData.matrix.userMode === "hotsim"
        )
          SR5_EntityHelpers.updateModifier(
            vehicleTest.limit,
            game.i18n.localize("SR5.VirtualReality"),
            game.i18n.localize("SR5.MatrixUserMode"),
            2
          );
        break;
      ...
      default:
        SR5_SystemHelpers.srLog(
          1,
          `Unknown controle mode '${data.controlMode}' in 'generateVehicleTest()'`
        );
    }
  ...
  static generateSpriteSkills(actor) {
    ...
    switch (data.type) {
      ...
      default:
        SR5_SystemHelpers.srLog(
          1,
          `Unknown '${data.type}' sprite type in '_generateSpriteSkills()'`
        );
        return;
    }
    ...
    for (let key of Object.keys(lists.skills)) {
      if (data.skills[key]) {
        if (data.skills[key].skillGroup) {
          ...
          if (data.skillGroups[linkedGroup].value) {
            if (data.skills[key].base === 0) {
              SR5_EntityHelpers.updateModifier(
                data.skills[key].rating,
                `${game.i18n.localize(lists.skillGroups[linkedGroup])}`,
                `${game.i18n.localize("SR5.SkillGroup")}`,
                data.skillGroups[linkedGroup].value
              );
            } else if (
              data.skillGroups[linkedGroup].value > data.skills[key].rating.base
            ) {
              let mod =
                data.skillGroups[linkedGroup].value -
                data.skills[key].rating.base;
              SR5_EntityHelpers.updateModifier(
                data.skills[key].rating,
                `${game.i18n.localize(lists.skillGroups[linkedGroup])}`,
                `${game.i18n.localize("SR5.SkillGroup")}`,
        ...
        if (
          ...
          let label = `${game.i18n.localize(
            lists.characterSpecialAttributes[linkedAttribute]
          )}`;
          SR5_EntityHelpers.updateModifier(
            data.skills[key].test,
            label,
            `${game.i18n.localize("SR5.LinkedAttribute")}`,
            data.specialAttributes[linkedAttribute].augmented.value
          );
        } else {
          let label = `${game.i18n.localize(
            lists.characterAttributes[linkedAttribute]
          )}`;
          SR5_EntityHelpers.updateModifier(
            data.skills[key].test,
            label,
            `${game.i18n.localize("SR5.LinkedAttribute")}`,
            data.attributes[linkedAttribute].augmented.value
          );
        }
        ...
        if (data.skills[key].rating.value) {
          ...
          data.skills[key].test.modifiers = data.skills[
            key
          ].test.modifiers.concat(data.skills[key].rating.modifiers);
          ...
          if (data.skills[key].canDefault) {
            ...
            SR5_EntityHelpers.updateModifier(
              data.skills[key].test,
              game.i18n.localize("SR5.Defaulting"),
              game.i18n.localize("SR5.Skill"),
              -1
            );
          } else {
        ...
        if (data.limits[linkedLimit]) {
          data.skills[key].limit.value =
            data.limits[linkedLimit].value +
            SR5_EntityHelpers.modifiersSum(data.skills[key].limit.modifiers);
        }
      }
    }
    ...
    if (actor.type !== "actorSprite") {
      for (let key of Object.keys(lists.spellCategories)) {
        if (data.skills.spellcasting.rating.value > 0) {
          data.skills.spellcasting.spellCategory[key].base =
            data.skills.spellcasting.rating.base;
          data.skills.spellcasting.spellCategory[key].modifiers =
            data.skills.spellcasting.spellCategory[key].modifiers.concat(
              data.skills.spellcasting.test.modifiers
            );
        }
        if (data.skills.counterspelling.rating.value > 0) {
          data.skills.counterspelling.spellCategory[key].base =
            data.skills.counterspelling.rating.base;
          data.skills.counterspelling.spellCategory[key].modifiers =
            data.skills.counterspelling.spellCategory[key].modifiers.concat(
              data.skills.counterspelling.test.modifiers
            );
        }
        if (data.skills.ritualSpellcasting.rating.value > 0) {
          data.skills.ritualSpellcasting.spellCategory[key].base =
            data.skills.ritualSpellcasting.rating.base;
          data.skills.ritualSpellcasting.spellCategory[key].modifiers =
            data.skills.ritualSpellcasting.spellCategory[key].modifiers.concat(
              data.skills.ritualSpellcasting.test.modifiers
            );
        }
        SR5_EntityHelpers.updateDicePool(
          data.skills.spellcasting.spellCategory[key],
          0
        );
        SR5_EntityHelpers.updateDicePool(
          data.skills.counterspelling.spellCategory[key],
          0
        );
        SR5_EntityHelpers.updateDicePool(
          data.skills.ritualSpellcasting.spellCategory[key],
          0
        );
      }

      for (let key of Object.keys(lists.spiritTypes)) {
        data.skills.summoning.spiritType[key].base =
          data.skills.summoning.test.dicePool;
        data.skills.binding.spiritType[key].base =
          data.skills.binding.test.dicePool;
        data.skills.banishing.spiritType[key].base =
          data.skills.banishing.test.dicePool;
        SR5_EntityHelpers.updateDicePool(
          data.skills.summoning.spiritType[key],
          0
        );
        SR5_EntityHelpers.updateDicePool(
          data.skills.binding.spiritType[key],
          0
        );
        SR5_EntityHelpers.updateDicePool(
          data.skills.banishing.spiritType[key],
          0
        );
      }
      ...
      for (let key of Object.keys(lists.perceptionTypes)) {
        SR5_EntityHelpers.updateValue(
          data.skills.perception.perceptionType[key].test,
          0
        );
        SR5_EntityHelpers.updateValue(
          data.skills.perception.perceptionType[key].limit,
          0
        );
      }
    }
  ...
  static _generateKnowledgeSkills(knowledge, actor) {
    ...
    switch (knowledge.type) {
      ...
      default:
        SR5_SystemHelpers.srLog(
          1,
          `Unknown '${knowledge.type}' knowledge type in '_generateKnowledgeSkills()'`
        );
        return;
    }

    let label = `${game.i18n.localize(
      lists.characterAttributes[knowledge.linkedAttribute]
    )}`;
    SR5_EntityHelpers.updateModifier(
      knowledge,
      label,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      attributes[knowledge.linkedAttribute].augmented.value
    );
  ...
  static updateTradition(actor) {
    ...
    if (!magic.magicType) {
      SR5_SystemHelpers.srLog(
        3,
        `Actor has no magic type selected or no magic capabilities for ${this.name}`
      );
      return;
    }
    ...
    switch (magic.magicType) {
      case "adept":
        ...
        magic.powerPoints.maximum.base =
          specialAttributes.magic.augmented.value;
      ...
      default:
        SR5_SystemHelpers.srLog(
          1,
          `Unknown '${magic.magicType}' magic type in updateTradition()`
        );
    }
  }
  ...
  static updateAstralValues(actor) {
    ...
    if (magic.magicType == "adept" || magic.magicType == "mysticalAdept") {
      if (items) {
        for (let item of Object.values(items)) {
          if (
            item.type === "itemAdeptPower" &&
            item.data.powerPointsCost.value
          ) {
            let label = `${item.name} (${game.i18n.localize(
              lists.itemTypes[item.type]
            )})`;
            SR5_EntityHelpers.updateModifier(
              magic.powerPoints,
              label,
              `itemAdeptPower_${item.id}`,
              item.data.powerPointsCost.value
            );
            SR5_EntityHelpers.updateValue(magic.powerPoints);
          }
    ...
    if (
      magic.magicType == "magician" ||
      magic.magicType == "aspectedMagician" ||
      magic.magicType == "mysticalAdept"
    ) {
      magic.passThroughBarrier.base = 0;
      SR5_EntityHelpers.updateModifier(
        magic.passThroughBarrier,
        `${game.i18n.localize("SR5.Charisma")}`,
        `${game.i18n.localize("SR5.LinkedAttribute")}`,
        attributes.charisma.augmented.value
      );
    ...
    if (magic.drainResistance.linkedAttribute) {
      let label = `${game.i18n.localize(
        lists.characterAttributes[magic.drainResistance.linkedAttribute]
      )}`;
      SR5_EntityHelpers.updateModifier(
        magic.drainResistance,
        label,
        `${game.i18n.localize("SR5.LinkedAttribute")}`,
        attributes[magic.drainResistance.linkedAttribute].augmented.value
      );
    }
    ...
    if (actor.type === "actorSpirit") {
      if (data.type === "homunculus" || data.type === "watcher") {
        SR5_EntityHelpers.updateModifier(
          magic.astralDamage,
          `${game.i18n.localize(lists.spiritTypes[spiritType])}`,
          `${game.i18n.localize("ACTOR.TypeActorspirit")}`,
          1
        );
      } else {
        SR5_EntityHelpers.updateModifier(
          magic.astralDamage,
          `${game.i18n.localize("SR5.SpiritForceShort")}`,
          `${game.i18n.localize("SR5.LinkedAttribute")}`,
          data.force.value
        );
      }
    ...
    if (skills.assensing.rating.value) {
      SR5_EntityHelpers.updateModifier(
        magic.astralTracking,
        `${game.i18n.localize("SR5.SkillAssensing")}`,
        `${game.i18n.localize("SR5.Skill")}`,
        skills.assensing.rating.value
      );
    }
  ...
  static generateMatrixAttributes(deck, actor) {
    ...
    switch (deck.data.type) {
      case "cyberdeck":
        matrix.attributesCollection.value1 =
          deck.data.attributesCollection.value1;
        matrix.attributesCollection.value2 =
          deck.data.attributesCollection.value2;
        matrix.attributesCollection.value3 =
          deck.data.attributesCollection.value3;
        matrix.attributesCollection.value4 =
          deck.data.attributesCollection.value4;
        matrix.programsMaximumActive.base = 0;
        SR5_EntityHelpers.updateModifier(
          matrix.programsMaximumActive,
          deck.name,
          `${game.i18n.localize("SR5.Device")}`,
          deck.data.program.max,
          false,
          false
        );
      ...
      case "riggerCommandConsole":
        ...
        matrix.attributes.dataProcessing.base =
          deck.data.attributesCollection.value3;
      ...
      case "livingPersona":
        ...
        break;
      case "headcase":
        matrix.attributes.attack.base =
          attributes.charisma.augmented.value +
          data.specialAttributes.resonance.augmented.value;
        matrix.attributes.sleaze.base =
          attributes.intuition.augmented.value +
          data.specialAttributes.resonance.augmented.value;
        matrix.attributes.dataProcessing.base =
          attributes.logic.augmented.value +
          data.specialAttributes.resonance.augmented.value;
        matrix.attributes.firewall.base =
          attributes.willpower.augmented.value +
          data.specialAttributes.resonance.augmented.value;
        matrix.deviceRating = data.specialAttributes.resonance.augmented.value;
        break;
      default:
        SR5_SystemHelpers.srLog(
          1,
          `Unknown '${deck.data.type}' deck type in generateMatrixAttributes()`
        );
        return;
    }
  ...
  static generateResonanceMatrix(deck, actor) {
    ...
    let data = actor.data,
      ...
      skills = data.skills;
    let matrix = data.matrix,
      resonanceActions = matrix.resonanceActions;

    resonanceActions.compileSprite.test.base = 0;
    SR5_EntityHelpers.updateModifier(
      resonanceActions.compileSprite.test,
      `${game.i18n.localize("SR5.Resonance")}`,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      specialAttributes.resonance.augmented.value
    );
    SR5_EntityHelpers.updateModifier(
      resonanceActions.compileSprite.test,
      `${game.i18n.localize("SR5.MatrixActionCompileSprite")}`,
      `${game.i18n.localize("SR5.Skill")}`,
      skills.compiling.rating.value
    );
    resonanceActions.decompileSprite.test.base = 0;
    SR5_EntityHelpers.updateModifier(
      resonanceActions.decompileSprite.test,
      `${game.i18n.localize("SR5.Resonance")}`,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      specialAttributes.resonance.augmented.value
    );
    SR5_EntityHelpers.updateModifier(
      resonanceActions.decompileSprite.test,
      `${game.i18n.localize("SR5.MatrixActionDecompileSprite")}`,
      `${game.i18n.localize("SR5.Skill")}`,
      skills.decompiling.rating.value
    );
    resonanceActions.eraseResonanceSignature.test.base = 0;
    SR5_EntityHelpers.updateModifier(
      resonanceActions.eraseResonanceSignature.test,
      `${game.i18n.localize("SR5.Resonance")}`,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      specialAttributes.resonance.augmented.value
    );
    SR5_EntityHelpers.updateModifier(
      resonanceActions.eraseResonanceSignature.test,
      `${game.i18n.localize("SR5.SkillComputer")}`,
      `${game.i18n.localize("SR5.Skill")}`,
      skills.computer.rating.value
    );
    resonanceActions.killComplexForm.test.base = 0;
    SR5_EntityHelpers.updateModifier(
      resonanceActions.killComplexForm.test,
      `${game.i18n.localize("SR5.Resonance")}`,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      specialAttributes.resonance.augmented.value
    );
    SR5_EntityHelpers.updateModifier(
      resonanceActions.killComplexForm.test,
      `${game.i18n.localize("SR5.SkillSoftware")}`,
      `${game.i18n.localize("SR5.Skill")}`,
      skills.software.rating.value
    );
    resonanceActions.registerSprite.test.base = 0;
    SR5_EntityHelpers.updateModifier(
      resonanceActions.registerSprite.test,
      `${game.i18n.localize("SR5.Resonance")}`,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      specialAttributes.resonance.augmented.value
    );
    SR5_EntityHelpers.updateModifier(
      resonanceActions.registerSprite.test,
      `${game.i18n.localize("SR5.SkillRegistering")}`,
      `${game.i18n.localize("SR5.Skill")}`,
      skills.registering.rating.value
    );
    resonanceActions.threadComplexForm.test.base = 0;
    SR5_EntityHelpers.updateModifier(
      resonanceActions.threadComplexForm.test,
      `${game.i18n.localize("SR5.Resonance")}`,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      specialAttributes.resonance.augmented.value
    );
    ...
    for (let key of Object.keys(lists.resonanceActions)) {
      if (resonanceActions[key].test) {
        ...
        if (resonanceActions[key].test.dicePool < 0)
          resonanceActions[key].test.dicePool = 0;
        //limit
  ...
  static generateMatrixActions(actor) {
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.jamSignals.test,
      game.i18n.localize("SR5.SkillElectronicWarfare"),
      game.i18n.localize("SR5.Skill"),
      skills.electronicWarfare.rating.value
    );
    SR5_EntityHelpers.updateModifier(
      matrixActions.jamSignals.test,
      game.i18n.localize("SR5.Logic"),
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.controlDevice.test,
      game.i18n.localize("SR5.SkillElectronicWarfare"),
      game.i18n.localize("SR5.Skill"),
      skills.electronicWarfare.rating.value
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.disarmDataBomb.test,
      game.i18n.localize("SR5.SkillSoftware"),
      game.i18n.localize("SR5.Skill"),
      skills.software.rating.value
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.formatDevice.test,
      game.i18n.localize("SR5.SkillComputer"),
      game.i18n.localize("SR5.Skill"),
      skills.computer.rating.value
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.hackOnTheFly.test,
      game.i18n.localize("SR5.SkillHacking"),
      game.i18n.localize("SR5.Skill"),
      skills.hacking.rating.value
    );
    SR5_EntityHelpers.updateModifier(
      matrixActions.hackOnTheFly.test,
      game.i18n.localize("SR5.Logic"),
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.spoofCommand.test,
      game.i18n.localize("SR5.SkillHacking"),
      game.i18n.localize("SR5.Skill"),
      skills.hacking.rating.value
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.garbageInGarbageOut.test,
      game.i18n.localize("SR5.SkillComputer"),
      game.i18n.localize("SR5.Skill"),
      skills.computer.rating.value
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.bruteForce.test,
      game.i18n.localize("SR5.SkillCybercombat"),
      game.i18n.localize("SR5.Skill"),
      skills.cybercombat.rating.value
    );
    SR5_EntityHelpers.updateModifier(
      matrixActions.bruteForce.test,
      game.i18n.localize("SR5.Logic"),
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.matrixPerception.test,
      game.i18n.localize("SR5.SkillComputer"),
      game.i18n.localize("SR5.Skill"),
      skills.computer.rating.value
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.crashProgram.test,
      game.i18n.localize("SR5.SkillCybercombat"),
      game.i18n.localize("SR5.Skill"),
      skills.cybercombat.rating.value
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.setDataBomb.test,
      game.i18n.localize("SR5.SkillSoftware"),
      game.i18n.localize("SR5.Skill"),
      skills.software.rating.value
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.jumpIntoRiggedDevice.test,
      game.i18n.localize("SR5.SkillElectronicWarfare"),
      game.i18n.localize("SR5.Skill"),
      skills.electronicWarfare.rating.value
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.rebootDevice.test,
      game.i18n.localize("SR5.SkillComputer"),
      game.i18n.localize("SR5.Skill"),
      skills.computer.rating.value
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.matrixSearch.test,
      game.i18n.localize("SR5.SkillComputer"),
      game.i18n.localize("SR5.Skill"),
      skills.computer.rating.value
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.jackOut.test,
      game.i18n.localize("SR5.SkillHardware"),
      game.i18n.localize("SR5.Skill"),
      skills.hardware.rating.value
    );
    SR5_EntityHelpers.updateModifier(
      matrixActions.jackOut.test,
      game.i18n.localize("SR5.Willpower"),
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.checkOverwatchScore.test,
      game.i18n.localize("SR5.SkillElectronicWarfare"),
      game.i18n.localize("SR5.Skill"),
      skills.electronicWarfare.rating.value
    );
    ...
    for (let key of Object.keys(lists.matrixActions)) {
      if (matrixActions[key].test !== undefined) {
        ...
        if (matrix.runningSilent) {
          SR5_EntityHelpers.updateModifier(
            matrixActions[key].test,
            game.i18n.localize("SR5.RunningSilent"),
            game.i18n.localize("SR5.MatrixUserMode"),
            -2
          );
        }
        ...
        SR5_EntityHelpers.updateModifier(
          matrixActions[key].limit,
          game.i18n.localize(lists.matrixAttributes[linkedAttribute]),
          game.i18n.localize("SR5.LinkedAttribute"),
          matrixAttributes[linkedAttribute].value
        );
  ...
  static generateMatrixActionsDefenses(actor) {
    ...
    if (
      actor.type === "actorPc" ||
      actor.type === "actorGrunt" ||
      actor.type === "actorAgent"
    ) {
    ...
    if (
      actor.type === "actorDevice" ||
      actor.type === "actorSprite" ||
      actor.type === "actorDrone"
    ) {
      if (
        (matrix.deviceType === "slavedDevice" && data.isDirectlyConnected) ||
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.editFile.defense,
      `${game.i18n.localize("SR5.Intuition")}`,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      intuitionValue
    );
    SR5_EntityHelpers.updateModifier(
      matrixActions.editFile.defense,
      `${game.i18n.localize("SR5.Firewall")}`,
      `${game.i18n.localize("SR5.MatrixAttribute")}`,
      firewallValue
    );
    SR5_EntityHelpers.updateModifier(
      matrixActions.eraseMark.defense,
      `${game.i18n.localize("SR5.Willpower")}`,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      willpowerValue
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.formatDevice.defense,
      `${game.i18n.localize("SR5.Willpower")}`,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      willpowerValue
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.hackOnTheFly.defense,
      `${game.i18n.localize("SR5.Intuition")}`,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      intuitionValue
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.spoofCommand.defense,
      `${game.i18n.localize("SR5.Logic")}`,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      logicValue
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.garbageInGarbageOut.defense,
      `${game.i18n.localize("SR5.Logic")}`,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      logicValue
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.bruteForce.defense,
      `${game.i18n.localize("SR5.Willpower")}`,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      willpowerValue
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.matrixPerception.defense,
      `${game.i18n.localize("SR5.Logic")}`,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      logicValue
    );
    SR5_EntityHelpers.updateModifier(
      matrixActions.matrixPerception.defense,
      `${game.i18n.localize("SR5.Sleaze")}`,
      `${game.i18n.localize("SR5.MatrixAttribute")}`,
      sleazeValue
    );
    SR5_EntityHelpers.updateModifier(
      matrixActions.dataSpike.defense,
      `${game.i18n.localize("SR5.Intuition")}`,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      intuitionValue
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.crashProgram.defense,
      `${game.i18n.localize("SR5.Intuition")}`,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      intuitionValue
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.jumpIntoRiggedDevice.defense,
      `${game.i18n.localize("SR5.Willpower")}`,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      willpowerValue
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.rebootDevice.defense,
      `${game.i18n.localize("SR5.Willpower")}`,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      willpowerValue
    );
    ...
    SR5_EntityHelpers.updateModifier(
      matrixActions.jackOut.defense,
      `${game.i18n.localize("SR5.MatrixAttack")}`,
      `${game.i18n.localize("SR5.MatrixAttribute")}`,
      attackValue
    );
    SR5_EntityHelpers.updateModifier(
      matrixActions.traceIcon.defense,
      `${game.i18n.localize("SR5.Willpower")}`,
      `${game.i18n.localize("SR5.LinkedAttribute")}`,
      willpowerValue
    );
    ...
    if (actor.type === "actorDrone") {
      if (data.vehicleOwner.id && data.slaved) {
        ...
        for (let key of Object.keys(lists.matrixActionsDefenses)) {
          data.matrix.actions[key].defense.base = 0;
          SR5_EntityHelpers.updateModifier(
            data.matrix.actions[key].defense,
            game.i18n.localize("SR5.DeviceRating"),
            game.i18n.localize("SR5.Controler"),
            controler.matrix.actions[key].defense.dicePool
          );
        }
      }
    }
  ...
  static generateMatrixResistances(actor, deck) {
    ...
    let matrix = data.matrix,
      matrixAttributes = matrix.attributes,
      matrixResistances = matrix.resistances;
    ...
    switch (deck.data.type) {
      case "commlink":
        SR5_EntityHelpers.updateModifier(
          matrixResistances.matrixDamage,
          deck.name,
          `${game.i18n.localize("SR5.DeviceRating")}`,
          deck.data.deviceRating
        );
        SR5_EntityHelpers.updateModifier(
          ...
          deck.name,
          `${game.i18n.localize("SR5.Firewall")}`,
          matrixAttributes.firewall.value
        );
        break;
      case "cyberdeck":
      case "riggerCommandConsole":
        ...
        if (actor.type === "actorDrone") {
          if (actor.data.vehicleOwner.id && data.slaved) {
            ...
            SR5_EntityHelpers.updateModifier(
              ...
              deck.name,
              `${game.i18n.localize("SR5.Controler")}`,
              controler.matrix.resistances.matrixDamage.dicePool
            );
          } else {
            SR5_EntityHelpers.updateModifier(
              matrixResistances.matrixDamage,
            ...
            SR5_EntityHelpers.updateModifier(
              ...
              deck.name,
              `${game.i18n.localize("SR5.Firewall")}`,
              data.matrix.attributes.firewall.value
            );
          }
          ...
          SR5_EntityHelpers.updateModifier(
            matrixResistances.matrixDamage,
            `${game.i18n.localize("SR5.ProgramTypeAgent")}`,
            `${game.i18n.localize("SR5.ItemRating")}`,
            data.rating
          );
          SR5_EntityHelpers.updateModifier(
            matrixResistances.matrixDamage,
      ...
      default:
        SR5_SystemHelpers.srLog(
          1,
          `Unknown '${deck.data.type}' deck type in generateMatrixResistances()`
        );
        return;
    }
  ...
  static generateVehicleMatrix(actor, deck) {
    ...
    data.matrix.programsMaximumActive.value = Math.ceil(
      data.matrix.deviceRating / 2
    );
  ...
  static generateDeviceMatrix(actor, deck) {
    ...
    if (matrix.deviceType === "ice") {
      ...
      matrix.actions.matrixPerception.limit.value =
        matrixAttributes.dataProcessing.value;
      switch (matrix.deviceSubType) {
        ...
        default:
          SR5_SystemHelpers.srLog(
            1,
            `Unknown '${matrix.deviceSubType}' ice type in generateDeviceMatrix()`
          );
      }
    }
  ...
  static generateSpriteMatrix(actor, deck) {
    let lists = actor.lists,
      data = actor.data,
      matrix = data.matrix;
    let matrixAttributes = matrix.attributes,
      matrixResistances = matrix.resistances,
      matrixActions = matrix.actions;
    ...
    switch (data.type) {
      case "courier":
        SR5_EntityHelpers.updateModifier(
          matrixAttributes.sleaze,
          label,
          `${game.i18n.localize("SR5.SpriteType")}`,
          +3
        );
        SR5_EntityHelpers.updateModifier(
          matrixAttributes.dataProcessing,
          label,
          `${game.i18n.localize("SR5.SpriteType")}`,
          +1
        );
        SR5_EntityHelpers.updateModifier(
          matrixAttributes.firewall,
          label,
          `${game.i18n.localize("SR5.SpriteType")}`,
          +2
        );
      ...
      default:
        SR5_SystemHelpers.srLog(
          1,
          `Unknown '${data.type}' sprite type in generateSpriteMatrix()`
        );
    }
  ...
  static generateAgentMatrix(actor, deck) {
    ...
    let matrixAttributes = actorData.matrix.attributes,
      creatorMatrix = actorData.creatorData.data.matrix;
  ...
  static async updateAgentOwner(agent) {
    ...
    let ownerDeck = owner.items.find(
      (i) => i.data.type === "itemDevice" && i.data.data.isActive
    );
    let newDeck = duplicate(ownerDeck);
    if (
      newDeck.data.conditionMonitors.matrix.current !==
      agent.data.conditionMonitors.matrix.current
    ) {
      newDeck.data.conditionMonitors.matrix =
        agent.data.conditionMonitors.matrix;
  ...
  static async updateAgent(actor, deck) {
    if (game.actors) {
      for (let a of game.actors) {
        if (
          a.data.type === "actorAgent" &&
          a.data.data.creatorId === actor._id
        ) {
          let agent = duplicate(a);
          agent.data.conditionMonitors.matrix =
            deck.data.conditionMonitors.matrix;
  ...
  static applyProgramToAgent(actor) {
    ...
    for (let i of actorData.creatorData.items) {
      if (
        i.type === "itemProgram" &&
        (i.data.type === "common" || i.data.type === "hacking")
      ) {
  ...
  static async updateControledVehicle(actor) {
    if (game.actors) {
      for (let a of game.actors) {
        if (
          a.data.type === "actorDrone" &&
          a.data.data.vehicleOwner.id === actor._id
        ) {
    ...
    if (canvas.scene) {
      for (let t of canvas.tokens.placeables) {
        if (
          t.actor.data.type === "actorDrone" &&
          t.actor.data.data.vehicleOwner.id === actor._id
        ) {
  ...
  static applyAutosoftEffect(actor) {
    ...
    let hasLocalAutosoftRunning = actor.items.find(
      (a) => a.data.data.type === "autosoft" && a.data.data.isActive
    );
    if (hasLocalAutosoftRunning)
      actor.data.matrix.hasLocalAutosoftRunning = true;
    ...
    if (actor.data.controlMode === "autopilot") {
      for (let i of controler.items) {
        if (
          i.type === "itemProgram" &&
          i.data.type === "autosoft" &&
          i.data.isActive &&
          !hasLocalAutosoftRunning
        ) {
  ...
  static _actorModifPossession(spirit, actor) {
    let lists = actor.lists,
      data = actor.data,
      actorAttribute = data.attributes;
    let spiritForce = spirit.data.data.itemRating,
      spiritType = spirit.data.data.type,
      spiritAttributes = spirit.data.data.attributes;

    // Attributes modifiers
    for (let key of Object.keys(lists.characterPhysicalAttributes)) {
      if (actorAttribute[key].augmented.base < spiritForce) {
        SR5_EntityHelpers.updateModifier(
          actorAttribute[key].augmented,
          `${game.i18n.localize("SR5.Possession")} (${game.i18n.localize(
            lists.spiritTypes[spiritType]
          )})`,
          "Possesion",
          Math.floor(spiritForce / 2)
        );
      }
    }
    ...
    for (let key of Object.keys(lists.characterSpecialAttributes)) {
      if (spiritAttributes[key]) {
        let mod =
          spiritAttributes[key] - data.specialAttributes[key].augmented.base;
        SR5_EntityHelpers.updateModifier(
          data.specialAttributes[key].augmented,
          `${game.i18n.localize("SR5.Possession")} (${game.i18n.localize(
            lists.spiritTypes[spiritType]
    ...
    SR5_EntityHelpers.updateModifier(
      data.penalties.condition.actual,
      `${game.i18n.localize("SR5.Possession")} (${game.i18n.localize(
        lists.spiritTypes[spiritType]
      )})`,
      "Possesion",
  ...
  static applyCustomEffects(item, actor) {
    ...
    for (let customEffect of Object.values(item.data.customEffects)) {
      ...
      if (!customEffect.target || !customEffect.type) {
        SR5_SystemHelpers.srLog(
          3,
          `Empty custom effect target or type in applyCustomEffects()`,
          customEffect
        );
      ...
      let targetObject = SR5_EntityHelpers.resolveObjectPath(
        customEffect.target,
        actor
      );
        ...
        if (customEffect.category === "weaponEffectTargets") {
          if (customEffect.target === "data.itemsProperties.weapon.accuracy") {
            ...
            SR5_EntityHelpers.updateModifier(
              ...
              customEffect.type,
              customEffect.value * customEffect.multiplier,
              isMultiplier,
              cumulative
            );
          }
          if (
            customEffect.target === "data.itemsProperties.weapon.damageValue"
          ) {
            customEffect.value = customEffect.value || 0;
            SR5_EntityHelpers.updateModifier(
              targetObject,
        ...
        switch (customEffect.type) {
          case "rating":
            ...
            SR5_EntityHelpers.updateModifier(
              targetObject,
              `${item.name}`,
              `${game.i18n.localize(lists.itemTypes[item.type])}`,
              customEffect.value * customEffect.multiplier,
              isMultiplier,
              cumulative
            );
          ...
          case "valueReplace":
            ...
            SR5_EntityHelpers.updateModifier(
              ...
              `${item.name}`,
              `${game.i18n.localize(lists.itemTypes[item.type])}`,
              modValue * customEffect.multiplier,
              isMultiplier,
              cumulative
            );
            break;
          ...
          case "ratingReplace":
            ...
            SR5_EntityHelpers.updateModifier(
              ...
              `${item.name}`,
              `${game.i18n.localize(lists.itemTypes[item.type])}`,
              modRating * customEffect.multiplier,
              isMultiplier,
              cumulative
            );
            break;
          ...
          default:
            SR5_SystemHelpers.srLog(
              1,
              `Unknown '${customEffect.type}' custom effect type in applyCustomEffects()`,
              customEffect
            );
        }
      }
