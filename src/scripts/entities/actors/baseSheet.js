...
export class ActorSheetSR5 extends ActorSheet {
  ...
  activateListeners(html) {
    ...
    html
      .find(".changeValueByClick")
      .mousedown(this._onChangeValueByClick.bind(this));
    //Choose controler
    ...
    html.find(".boxes:not(.box-disabled)").click((ev) => {
      ...
      if (
        target == "data.conditionMonitors.physical.current" &&
        getProperty(actorData, "data.conditionMonitors.overflow.current")
      ) {
        if (
  ...
  async _onDrop(event) {
    ...
    if (dropData.valueFromCollection) {
      ...
      if (existingValue > 0) {
        for (let [key, value] of Object.entries(
          actorData.data.matrix.attributesCollection
        )) {
          if (value === existingValue) {
            setProperty(
              actorData,
              `data.matrix.attributesCollection.${key}isSet`,
              false
            );
            break;
          }
      ...
      setProperty(
        actorData,
        target.dataset.dropmatrixattribute,
        parseInt(dropData.value)
      );
      setProperty(
        actorData,
        `data.matrix.attributesCollection.${dropData.valueFromCollection}`,
        true
      );
    ...
    if (dropData.valueFromAttribute) {
      ...
      setProperty(
        actorData,
        dropData.valueFromAttribute,
        parseInt(target.dataset.droppedvalue)
      );
      await this.actor.update(actorData);
    }
  ...
  _onItemCreate(event) {
    ...
    SR5_SystemHelpers.srLog(
      2,
      `Creating a new item of '${type}' type`,
      itemData
    );
  ...
  _onItemClone(event) {
    ...
    SR5_SystemHelpers.srLog(
      2,
      `Creating a new clone of item '${item.name}'`,
      item
    );
  ...
  async _onItemDelete(event) {
    ...
    if (event.ctrlKey) {
      if (item) {
        await item.delete();
        let statusEffect = this.actor.effects.find(
          (e) => e.data.origin === item.data.data.type
        );
        if (statusEffect)
          this.actor.deleteEmbeddedDocuments("ActiveEffect", [statusEffect.id]);
        return;
      }
    } else {
      Dialog.confirm({
        title: `${game.i18n.localize("SR5.Delete")} '${
          item.name
        }'${game.i18n.localize("SR5.QuestionMark")}`,
        content:
          "<h3>" +
          game.i18n.localize("SR5.DIALOG_Warning") +
          "</h3><p>" +
          game.i18n.format("SR5.DIALOG_WarningPermanentDelete", {
            type: game.i18n.localize(
              "ITEM.Type" + item.type.replace(/^\w/, (c) => c.toUpperCase())
            ),
  ...
  _onItemSummary(event) {
    ...
    if (li.hasClass("expanded")) {
      ...
      let accessoryClass = $(event.currentTarget).hasClass("SR-MarginLeft10")
        ? "SR-MarginLeft10"
        : "";
      let div = $(
        `<div class="col-x item-summary ${accessoryClass}">${expandData.gameEffect}</div>`
      );
      ...
      expandData.properties.forEach((p) => {
        if (Array.isArray(p)) {
          ...
          props.append(
            `<span class="tag tag-summary" data-index=${index}>${p[0]}</span>`
          );
        } else {
      ...
      div.find(".tag-summary").click((event) => {
        ...
        let tagDiv = $(
          `<div class="item-properties tag-description ${i}">${gameEffect}</div>`
        );
  ...
  async _onEditItemValue(event) {
    ...
    if (
      item.type === "itemDevice" &&
      target !== "data.conditionMonitors.matrix.current"
    ) {
      for (let otherItem of itemList) {
        if (otherItem.type === "itemDevice" && otherItem._id !== id)
          otherItem.data.isActive = false;
      }
    ...
    if (
      target === "data.isActive" &&
      item.type === "itemArmor" &&
      !item.data.isCumulative
    ) {
      for (let otherItem of itemList) {
        if (
          otherItem.type === "itemArmor" &&
          !otherItem.data.isCumulative &&
          otherItem._id !== id
        )
          otherItem.data.isActive = false;
      }
  ...
  _onChangeValueByClick(event) {
    ...
    switch (event.button) {
      ...
      case 2:
        if (
          target === "data.matrix.attributes.sharing.base" ||
          target === "data.matrix.attributes.noiseReduction.base"
        ) {
          let deviceRating = entity.data.matrix.deviceRating;
          let noiseReduction =
            entity.data.matrix.attributes.noiseReduction.base;
          ...
          if (target === "data.matrix.attributes.sharing.base") {
            if (value + noiseReduction < deviceRating) {
              ...
              if (noiseReduction > 0) {
                setProperty(
                  entity,
                  "data.matrix.attributes.noiseReduction.base",
                  noiseReduction - 1
                );
                value++;
              } else {
          ...
          if (target === "data.matrix.attributes.noiseReduction.base") {
            if (value + sharing < deviceRating) {
              ...
              if (sharing > 0) {
                setProperty(
                  entity,
                  "data.matrix.attributes.sharing.base",
                  sharing - 1
                );
                value++;
              } else {
  ...
  async _displayHelpText(event) {
    ...
    if (target) {
      ...
      if ($(event.currentTarget).attr("data-helpMessage"))
        document.querySelector("#sr5helpMessage").innerHTML =
          "<div class='helpMessage'><em>" +
          $(event.currentTarget).attr("data-helpMessage") +
          "</em></div>";

      let details = $(event.currentTarget).attr("data-helpDetails");
      if (details) {
        property = SR5_EntityHelpers.resolveObjectPath(
          `actor.data.${details}`,
          this
        );
      }
      ...
      if (property) {
        let detailsHTML = `${game.i18n.localize(
          "SR5.HELP_CalculationDetails"
        )}<ul>`;
        if (property.modifiers && property.modifiers.length) {
          if (property.base)
            detailsHTML += `<li>${game.i18n.localize(
              "SR5.HELP_CalculationBase"
            )}${game.i18n.localize("SR5.Colons")} ${property.base}</li>`;
          for (let modifier of Object.values(property.modifiers)) {
            if (modifier.type === "armorAccessory")
              modifier.type = game.i18n.localize("SR5.ArmorAccessory");
            if (modifier.type === "armorMain")
              modifier.type = game.i18n.localize("SR5.Armor");
            if (modifier.value != undefined)
              detailsHTML =
                detailsHTML +
                `<li>${modifier.source} [${modifier.type}]${game.i18n.localize(
                  "SR5.Colons"
                )} ${
                  modifier.isMultiplier ? "x" : modifier.value >= 0 ? "+" : ""
                }${modifier.value}</li>`;
            else
              detailsHTML =
                detailsHTML +
                `<li>${modifier.source} [${modifier.type}]${game.i18n.localize(
                  ...
                  modifier.isMultiplier
                    ? "x"
                    : modifier.dicePool >= 0
                    ? "+"
                    : ""
                }${modifier.dicePool}</li>`;
          }
        }
        if (property.value != undefined)
          detailsHTML += `<li>${game.i18n.localize(
            "SR5.HELP_CalculationTotal"
          )}${game.i18n.localize("SR5.Colons")} ${property.value}</li></ul>`;
        else
          detailsHTML += `<li>${game.i18n.localize(
            "SR5.HELP_CalculationTotal"
          )}${game.i18n.localize("SR5.Colons")} ${property.dicePool}</li></ul>`;
  ...
  async _onChooseControler(event) {
    ...
    for (let a of game.actors) {
      if (
        a.data.type === "actorPc" ||
        (a.data.type === "actorGrunt" && a.data.token.actorLink)
      ) {
        if (game.user.isGM) {
    ...
    renderTemplate(
      "systems/sr5/templates/interface/chooseControler.html",
      dialogData
    ).then((dlg) => {
      new Dialog({
        ...
        close: (html) => {
          ...
          if (controler) {
            ...
            let vehicleControler =
              SR5_EntityHelpers.getRealActorFromID(controler);
            this.actor.update({
              "data.vehicleOwner.id": controler,
              "data.vehicleOwner.name": controlerName,
              "flags.sr5.vehicleControler":
                vehicleControler.data.toObject(false),
            });
          } else {
            this.actor.update({
  ...
  async _onAddItemToPan(event) {
    ...
    if (actor.data.matrix.pan.current === actor.data.matrix.pan.max) {
      ui.notifications.info(
        `${actor.name}: ${game.i18n.localize("SR5.INFO_PanIsFull")}`
      );
      return;
    }
    ...
    for (let key of Object.keys(actor.data.matrix.potentialPanObject)) {
      if (Object.keys(actor.data.matrix.potentialPanObject[key]).length) {
        list[key] = SR5_EntityHelpers.sortObjectValue(
          actor.data.matrix.potentialPanObject[key]
        );
      }
    }
    ...
    renderTemplate(
      "systems/sr5/templates/interface/addItemToPan.html",
      dialogData
    ).then((dlg) => {
      new SR5_PanDialog({
  ...
  async _onStopJamming(event) {
    event.preventDefault();
    let jammingItem = this.actor.items.find(
      (i) => i.data.data.type === "signalJam"
    );
    let statusEffect = this.actor.effects.find(
      (e) => e.data.origin === "signalJam"
    );
    if (statusEffect)
