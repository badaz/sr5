...
export class SR5_RollMessage {
  ...
  static async chatListeners(html, data) {
    ...
    if (!game.user.isGM) {
      if (
        data.message.speaker.actor &&
        game.actors.get(data.message.speaker.actor)?.permission != 3
      ) {
    ...
    html.on("click", ".edit-toggle", (ev) => {
      ev.preventDefault();
      let elementsToToggle = $(ev.currentTarget)
        .parents(".chat-card")
        .find(".display-toggle");
      if (!elementsToToggle.length)
        elementsToToggle = $(ev.currentTarget).find(".display-toggle");
    ...
    html.on("change", ".card-edit", async (ev) => {
      let button = $(ev.currentTarget),
        ...
        actor = SR5_EntityHelpers.getRealActorFromID(
          message.data.flags.speakerId
        ),
        newMessage = duplicate(message.data.flags.sr5data);

      newMessage.test[button.attr("data-edit-type")] = parseInt(
        ev.target.value
      );
      ...
      if (newMessage.originalMessage) {
        updatedOriginalMessage = await game.messages.get(
          newMessage.originalMessage._id
        );
  ...
  static async chatButtonAction(ev) {
      ...
      switch (type) {
        ...
        case "msgTest_matrixDefense":
          if (
            ...
            actor.data.type !== "actorDevice" &&
            actor.data.type !== "actorSprite" &&
            actor.data.type !== "actorDrone" &&
            actor.data.type !== "actorAgent" &&
    ...
    if (action === "nonOpposedTest" && messageData) {
      ...
      if (messageData.originalActionAuthor) {
        originalActionAuthor = SR5_EntityHelpers.getRealActorFromID(
          messageData.originalActionAuthor
        );
      }
      ...
      switch (type) {
        ...
        case "msgTest_matrixResistance":
          actor.rollTest(
            "matrixResistance",
            messageData.matrixResistanceType,
            messageData
          );
          break;
        ...
        case "msgTest_createSpirit":
          ...
          ui.notifications.info(
            `${actor.name} ${game.i18n.localize(
              "SR5.INFO_SummonSpirit"
            )} ${game.i18n.localize(
              SR5.spiritTypes[messageData.spiritType]
            )} (${messageData.force})`
          );
        ...
        case "msgTest_createSprite":
          ...
          ui.notifications.info(
            `${actor.name} ${game.i18n.localize(
              "SR5.INFO_CompileSprite"
            )} ${game.i18n.localize(
              SR5.spriteTypes[messageData.spriteType]
            )} (${messageData.level})`
          );
        ...
        case "msgTest_createPreparation":
          ...
          ui.notifications.info(
            `${actor.name} ${game.i18n.localize(
              "SR5.INFO_CreatePreparation"
            )} ${messageData.item.name}`
          );
        ...
        case "msgTest_attackerAddMark":
          await SR5_DiceHelper.markItem(
            actor,
            messageData.originalActionAuthor,
            messageData.mark,
            messageData.matrixTargetItem
          );
          // if defender is a slaved device, add mark to host
          if (
            actor.data.data.matrix.deviceType === "slavedDevice" ||
            actor.data.data.matrix.deviceType === "ice"
          ) {
            for (let server of game.actors) {
              if (
                server.id === actor.id &&
                server.data.data.matrix.deviceType === "host"
              ) {
                await SR5_DiceHelper.markDevice(
                  server.id,
                  messageData.originalActionAuthor,
                  messageData.mark
                );
          ...
          if (actor.data.type === "actorDrone" && actor.data.data.slaved) {
            if (!game.user?.isGM) {
              ...
              await SR5_DiceHelper.markDevice(
                actor.data.data.vehicleOwner.id,
                messageData.originalActionAuthor,
                messageData.mark
              );
            }
        ...
        case "msgTest_defenderDoDeckDamage":
          if (
            originalActionAuthor.data.type === "actorPc" ||
            originalActionAuthor.data.type === "actorGrunt"
          ) {
            if (
              originalActionAuthor.items.find(
                (item) =>
                  item.type === "itemDevice" &&
                  item.data.data.isActive &&
                  (item.data.data.type === "livingPersona" ||
                    item.data.data.type === "headcase")
              )
            ) {
              originalActionAuthor.takeDamage(messageData);
            } else {
              SR5_DiceHelper.applyDamageToDecK(
                originalActionAuthor,
                messageData,
                actor
              );
            }
          ...
          SR5_RollMessage.updateChatButton(
            message.data,
            "defenderDoMatrixDamage"
          );
          break;
        case "msgTest_takeMatrixDamage":
          if (
            actor.data.type === "actorPc" ||
            actor.data.type === "actorGrunt"
          ) {
            if (
              actor.items.find(
                (item) =>
  ...
  static async updateChatButton(message, buttonToUpdate) {
    ...
    if (
      message.data?.flags?.sr5data?.typeSub === "grenade" &&
      buttonToUpdate !== "scatter"
    )
      return;
    ...
    switch (buttonToUpdate) {
      ...
      case "eraseMarkSuccess":
        ...
        newMessage.button.actionEndTitle = game.i18n.localize(
          "SR5.MatrixActionEraseMarkSuccess"
        );
        break;
      case "reduceTask":
        newMessage.button.actionEnd = true;
        if (newMessage.actor.data.tasks.value + newMessage.netHits <= 0) {
          newMessage.button.actionEndTitle = game.i18n.localize(
            "SR5.DecompiledSprite"
          );
        } else
          newMessage.button.actionEndTitle = `${game.i18n.format(
            "SR5.INFO_TasksReduced",
            { task: newMessage.netHits }
          )}`;
      default:
    }
    await SR5_RollMessage.updateRollCard(message.data, newMessage);
  ...
  static async removeTemplate(message, itemId) {
    ...
    let template = canvas.scene.data.templates.find(
      (t) => t.data.flags.item === itemId
    );
    if (template) {
      ...
      ui.notifications.warn(
        `${game.i18n.localize("SR5.WARN_NoTemplateInScene")}`
      );
      if (message)
        SR5_RollMessage.updateChatButton(message.data, "removeTemplate");
    }
  ...
  static async buildSpirit(messageData) {
    let spirit = {
      name: `${game.i18n.localize("SR5.SummonedSpirit")} (${game.i18n.localize(
        SR5.spiritTypes[messageData.spiritType]
      )}, ${messageData.force})`,
      type: "itemSpirit",
      ...
      ["data.summonerMagic"]:
        messageData.actor.data.specialAttributes.magic.augmented.value,
    };
    return spirit;
  }
  ...
  static async buildSprite(messageData) {
    let sprite = {
      name: `${game.i18n.localize("SR5.CompiledSprite")} (${game.i18n.localize(
        SR5.spriteTypes[messageData.spriteType]
      )}, ${messageData.level})`,
      type: "itemSprite",
      ...
      ["data.compilerResonance"]:
        messageData.actor.data.specialAttributes.resonance.augmented.value,
    };
    return sprite;
  }
  ...
  static async buildPreparation(messageData) {
    ...
    preparation = mergeObject(preparation, {
      name: `${game.i18n.localize("SR5.Preparation")}${game.i18n.localize(
        "SR5.Colons"
      )} ${messageData.item.name}`,
