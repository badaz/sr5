export class SR5Token extends Token {
  ...
  get isVisible() {
    ...
    return canvas.sight.testVisibility(
      this.center,
      { tolerance, object: this },
      this
    );
  }
  ...
  _drawBar(number, bar, data) {
    ...
    let mainColorRGB = window
      .getComputedStyle(mainColorElement, null)
      .getPropertyValue("border-color");
    let mainColorArray = mainColorRGB
      .slice(mainColorRGB.indexOf("(") + 1, mainColorRGB.indexOf(")"))
      .split(", ");
    let mainColor = mainColorArray.map(function convertToFloat(number) {
      return number / 255;
    });
    let subColorElement = document.getElementById("sidebar");
    let subColorRGB = window
      .getComputedStyle(subColorElement, null)
      .getPropertyValue("border-left-color");
    let subColorArray = subColorRGB
      .slice(subColorRGB.indexOf("(") + 1, subColorRGB.indexOf(")"))
      .split(", ");
    ...
    bar
      .clear()
      .beginFill(PIXI.utils.rgb2hex(subColor), 0.7)
    ...
    bar
      .beginFill(PIXI.utils.rgb2hex(mainColor), 0.7)
      .lineStyle(0.5, 0x000000, 1);
  ...
  async _drawOverlay({ src, tint } = {}) {
    ...
    const bgtex = await loadTexture(
      "systems/sr5/assets/img/ui/TokenOverlay.png"
    );
    ...
    sprite.position.set(
      this.data.width * canvas.grid.w * 0.5,
      this.data.height * canvas.grid.h * 0.5
    );
  ...
  static async addTokenLayer(tokenDocument) {
    ...
    if (tokenDocument.actor.type === "actorGrunt") {
      tokenOverlay = await loadTexture(
        "systems/sr5/assets/img/ui/TokenLayerGrunt.png"
      );
    } else if (
      tokenDocument.actor.type === "actorSprite" ||
      tokenDocument.actor.type === "actorDevice"
    ) {
      tokenOverlay = await loadTexture(
        "systems/sr5/assets/img/ui/TokenLayerSprite.png"
      );
    } else {
      tokenOverlay = await loadTexture(
        "systems/sr5/assets/img/ui/TokenLayerPC.png"
      );
    }
    ...
    await icon.position.set(
      container.data.width * canvas.grid.w * 0.5,
      container.data.height * canvas.grid.h * 0.5
    );
    icon.zIndex = 1000;
  }
}
