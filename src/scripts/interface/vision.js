...
export class SR5SightLayer extends SightLayer {
  ...
  testVisibility(point, { tolerance = 2, object = null } = {}, caller) {
    ...
    if (!this._inBuffer && !points.some((p) => d.sceneRect.contains(p.x, p.y)))
      return false;
    ...
    for (let source of visionSources.values()) {
      ...
      if (caller) {
        ...
        let actor = SR5_EntityHelpers.getRealActorFromID(
          source.object.document.id
        );
        if (
          callerData.data?.initiatives?.astralInit?.isActive &&
          !actor.data.data?.visions?.astral.isActive
        )
          return false;
      }
      //End of Override
...
export const drawSight = function () {
  ...
  if (
    this.object?.document?.actor?.data?.data?.vision?.astral &&
    game.settings.get("sr5", "sr5AstralOverlay")
  ) {
    let bgtex = PIXI.Texture.from("systems/sr5/assets/img/ui/astral.jpg");
    fov
      .beginTextureFill({
        texture: bgtex,
        alpha: 0.8,
